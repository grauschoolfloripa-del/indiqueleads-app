...
import { 
  Building2, PlusCircle, LayoutDashboard, Landmark, Users, Upload, Check, AlertCircle, MapPin,
  ArrowRight, ShieldCheck, DollarSign, ExternalLink, RefreshCw, FileText, Calendar, Sparkles,
  Percent, FileSpreadsheet, Send, TrendingUp, Clock, Ban, Smartphone, Share2, MessageSquare,
  Database, Layers, Settings2, Activity, Trash2, HelpCircle, FileUp, ArrowRightLeft, Link, Code
} from 'lucide-react';
import { Product, Advertiser, Lead, Category, Indicator, ProductStatus, FinancingSimulation, FinancingStatus, BankSimulationResponse, ApprovedContract, ChatMessage } from '../types';
...
interface AdvertiserDashboardProps {
  ...
  onUpdateSimulationStatus: (simId: string, status: FinancingStatus, bankResponses?: BankSimulationResponse[], approvedContract?: ApprovedContract) => void;
  onUpdateLeadStatus: (leadId: string, status: any, extra?: { visitDate?: string; notes?: string; checkInRequested?: boolean }) => void;
...
export default function AdvertiserDashboard({
  ...
  // Navigation
  const [activeTab, setActiveTab] = useState<'funnel' | 'produtos' | 'financeiro' | 'afiliados' | 'financiamentos'>('funnel');
  ...
  const [integrations, setIntegrations] = useState({
    ...
    kenlo: { active: false, clientId: '', clientSecret: '', agencyId: '', autoSync: true, lastSync: '' },
  ...
  const [activeIntegrationDetail, setActiveIntegrationDetail] = useState<'vistaCrm' | 'kenlo' | 'webmotors' | 'bling' | 'shopify' | 'api_doc' | null>(null);
  const [isSyncing, setIsSyncing] = useState<boolean>(false);
  const [apiToken] = useState<string>(`indica_token_${advertiser?.id || '123'}_${Math.random().toString(36).substring(2, 10).toUpperCase()}`);
  ...
  const [whatsAppNotificationData, setWhatsAppNotificationData] = useState<{ indicatorPhone: string; indicatorName: string; text: string } | null>(null);
  ...
  const handleRegisterAdvertiser = (e: FormEvent) => {
    ...
    onAddNotification('Conta de Anunciante cadastrada com sucesso! Seu catálogo está liberado.', 'success');
  };
    ...
    const responses: BankSimulationResponse[] = [
      {
        ...
        installmentValue: Math.round((totalToFinance * (1 + (standardRateItau / 100) * sim.desiredInstallments)) / sim.desiredInstallments),
      ...
      },
      {
        ...
        installmentsCount: sim.desiredInstallments,
        installmentValue: Math.round((totalToFinance * (1 + (standardRateBradesco / 100) * sim.desiredInstallments)) / sim.desiredInstallments),
        approvedStatus: 'aprovado'
      },
      {
        ...
        installmentsCount: sim.desiredInstallments,
        installmentValue: Math.round(((totalToFinance - 5000) * (1 + (standardRateBV / 100) * sim.desiredInstallments)) / sim.desiredInstallments),
        approvedStatus: 'revisar_entrada'
      }
    ];

    onUpdateSimulationStatus(simId, 'analise_bancos', responses);
    onAddNotification('Ficha de crédito enviada para simulação multi-banco com sucesso!', 'success');
  };
  ...
  const handleOpenEditModal = (sim: FinancingSimulation) => {
    ...
    setApprovedContractForm({
      ...
      approvedAmount: sim.approvedContract?.approvedAmount.toString() || (sim.productPrice - sim.downPayment).toString(),
      downPaymentRequired: sim.approvedContract?.downPaymentRequired.toString() || sim.downPayment.toString(),
      installmentsCount: sim.approvedContract?.installmentsCount || sim.desiredInstallments,
      installmentValue: sim.approvedContract?.installmentValue.toString() || Math.round(((sim.productPrice - sim.downPayment) * 1.5) / sim.desiredInstallments).toString(),
      interestRate: sim.approvedContract?.interestRate.toString() || '1.49',
      additionalNotes: sim.approvedContract?.additionalNotes || 'Crédito aprovado! Favor enviar RG, CPF e comprovante de renda atualizados para formalização digital.'
    });
  ...
  const triggerIndicatorNotification = (simId: string, status: FinancingStatus, approvedContract?: ApprovedContract) => {
    const sim = simulations.find(s => s.id === simId);
    if (!sim) return;
  ...
  const handleAddProductSubmit = (e: FormEvent) => {
    ...
    const newProd: Product = {
      ...
      commissionPresencialValue: productForm.allowPresencialTier ? (priceVal * presencialPct) / 100 : undefined,
    ...
    onAddNotification(`Produto "${newProd.title}" adicionado e disponível para indicação!`, 'success');
  ...
  const parseBulkText = (text: string) => {
    ...
    if (lines.length < 2) {
      onAddNotification('Por favor, inclua pelo menos uma linha de cabeçalho e uma linha de dados.', 'info');
      return;
    }
    ...
    headers.forEach(h => {
      ...
      if (lower.includes('tit') || lower.includes('nome') || lower.includes('title')) initialMapping.title = h;
      else if (lower.includes('pre') || lower.includes('val') || lower.includes('price')) initialMapping.price = h;
      else if (lower.includes('des') || lower.includes('det') || lower.includes('desc')) initialMapping.description = h;
      else if (lower.includes('cid') || lower.includes('city')) initialMapping.city = h;
      else if (lower.includes('est') || lower.includes('uf') || lower.includes('state')) initialMapping.state = h;
      else if (lower.includes('img') || lower.includes('capa') || lower.includes('imag') || lower.includes('photo') || lower.includes('url')) initialMapping.coverImage = h;
    });
  ...
  const handleImportBulkProducts = () => {
    ...
    bulkParsedRows.forEach((row, idx) => {
      ...
      const description = bulkMapping.description ? (row.data[bulkMapping.description] || '') : 'Sem descrição importada.';
      ...
      let coverImage = 'https://images.unsplash.com/photo-1613490493576-7fde63acd811?auto=format&fit=crop&w=800&q=80';
      if (bulkMapping.coverImage && row.data[bulkMapping.coverImage]) {
        ...
        if (bulkCategory === 'carro') coverImage = 'https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&w=800&q=80';
        else if (bulkCategory === 'moto') coverImage = 'https://images.unsplash.com/photo-1558981806-ec527fa84c39?auto=format&fit=crop&w=800&q=80';
        else if (bulkCategory === 'barco') coverImage = 'https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?auto=format&fit=crop&w=800&q=80';
        else if (bulkCategory === 'jetski') coverImage = 'https://images.unsplash.com/photo-1633534591456-e9e034ea3efb?auto=format&fit=crop&w=800&q=80';
      }
    ...
    onAddNotification(`${successCount} produtos importados com sucesso para a categoria ${bulkCategory.toUpperCase()}!`, 'success');
  ...
  const handleTriggerSync = (platform: 'vistaCrm' | 'kenlo' | 'webmotors' | 'bling' | 'shopify') => {
    setIsSyncing(true);
    ...
    setTimeout(() => {
      ...
      const appendProduct = (title: string, price: number, description: string, city: string, state: string, category: Category, cover: string) => {
        const newProd: Product = {
      ...
      if (platform === 'vistaCrm') {
        appendProduct("Apartamento de Alto Padrão no Itaim Bibi", 2450000, "Excelente imóvel com 3 suítes, 4 vagas de garagem, varanda gourmet envidraçada e acabamento de luxo.", "São Paulo", "SP", "imovel", "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=800&q=80");
        appendProduct("Casa Duplex em Condomínio Fechado Barra", 3890000, "Maravilhosa casa com projeto contemporâneo, 5 suítes, piscina, área gourmet integrada e segurança 24h.", "Rio de Janeiro", "RJ", "imovel", "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=800&q=80");
      } else if (platform === 'kenlo') {
        appendProduct("Cobertura Penthouse com Vista para o Mar", 5200000, "Cobertura exclusiva reformada, piscina privativa com borda infinita, amplo deck de madeira e churrasqueira.", "Santos", "SP", "imovel", "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=800&q=80");
        appendProduct("Apartamento Studio Mobiliado na Consolação", 420000, "Studio decorado por arquiteto, ar-condicionado, eletrodomésticos premium, condomínio com lazer completo no rooftop.", "São Paulo", "SP", "imovel", "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=800&q=80");
      } else if (platform === 'webmotors') {
        appendProduct("Porsche 911 Carrera S Coupé 2022", 890000, "Único dono, todas as revisões feitas na concessionária, teto solar elétrico, interior em couro bicolor, apenas 12.000km.", "São Paulo", "SP", "carro", "https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?auto=format&fit=crop&w=800&q=80");
        appendProduct("BMW M3 Competition Track Pack 2021", 699000, "Motor de 510cv, freios de cerâmica, bancos concha em carbono, película de proteção PPF integral.", "Curitiba", "PR", "carro", "https://images.unsplash.com/photo-1555215695-3004980ad54e?auto=format&fit=crop&w=800&q=80");
      } else if (platform === 'bling') {
        appendProduct("Lancha NX Boats NX 360 Sport Coupé 2022", 780000, "Equipada com 2 motores Volvo Penta 250hp, gerador, ar-condicionado, churrasqueira elétrica, som premium Hertz.", "Angra dos Reis", "RJ", "barco", "https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?auto=format&fit=crop&w=800&q=80");
        appendProduct("Jet Ski Yamaha VX Cruiser HO 2023", 145000, "Edição de luxo com sistema de som JetSound integrado, GPS Garmin, assento Ergo-Fit de 3 lugares.", "Campinas", "SP", "jetski", "https://images.unsplash.com/photo-1633534591456-e9e034ea3efb?auto=format&fit=crop&w=800&q=80");
      } else if (platform === 'shopify') {
        appendProduct("Moto Harley-Davidson Fat Boy 114 2023", 115000, "Cor Vivid Black, motor Milwaukee-Eight 114, rodas de alumínio fundido Lakeside, escapamento Vance & Hines.", "Belo Horizonte", "MG", "moto", "https://images.unsplash.com/photo-1558981806-ec527fa84c39?auto=format&fit=crop&w=800&q=80");
      }
  ...
  const handleCloseSaleSubmit = (e: FormEvent) => {
    ...
    onAddNotification(`Venda de "${closingSaleLead.productTitle}" concluída! Comissão de R$ ${closingSaleLead.commissionValue.toLocaleString()} liberada para ${closingSaleLead.indicatorName}.`, 'success');
      ...
      <div className="max-w-md mx-auto my-8 bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 font-sans">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Cadastre seus bens imobiliários ou automotivos e utilize nossa rede de divulgadores autônomos por performance.
          </p>
        </div>

        <form onSubmit={handleRegisterAdvertiser} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Razão Social / Nome Fantasia</label>
            <input 
              type="text" required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Tipo de Conta</label>
              <select
                value={onboardForm.type}
                onChange={e => setOnboardForm({...onboardForm, type: e.target.value as 'PF' | 'PJ'})}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">CNPJ / CPF</label>
              <input 
                type="text" required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Telefone Comercial</label>
              <input 
                type="text" required
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">E-mail Corporativo</label>
              <input 
                type="email" required
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Cidade da Sede / Loja</label>
              <input 
                type="text" required
            ...
            <div className="col-span-1">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Estado (UF)</label>
              <select 
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Escolha seu Plano Mensal de Acesso</label>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider">Contrato de Credenciamento & Adesão do Anunciante</label>
              {scrolledTerms && <span className="text-[10px] text-emerald-600 font-bold bg-emerald-100 px-2 py-0.5 rounded-full">✓ Lido</span>}
            </div>
            ...
            >
              <p className="font-bold text-white mb-2 uppercase text-center border-b border-slate-800 pb-1">CONTRATO DE ADESÃO DO ANUNCIANTE</p>
              <p className="mb-2">Ao registrar sua conta corporativa, o Anunciante se compromete a reportar com fidelidade o encerramento de qualquer negociação iniciada por indicação recebida na plataforma IndicaAqui.</p>
              <p className="mb-2"><strong>DA COMISSÃO DEVIDA:</strong> O Anunciante se obriga a efetuar o repasse financeiro do comissionamento pactuado por lead/venda em até 7 (sete) dias após o recebimento dos valores contratuais do cliente indicado.</p>
              <p className="mb-2"><strong>DA PENALIDADE DE EXCLUSÃO:</strong> Lojas ou imobiliárias que negarem o crédito de atribuição de venda de indicadores ou que sonegarem comprovantes de fechamento serão banidas permanentemente, sem reembolso das mensalidades pagas.</p>
              <p className="text-orange-300 font-bold text-center mt-2">=== ROLAR ATÉ O FIM PARA ATIVAR O ACEITE ===</p>
            </div>
          </div>
  ...
  const pendingArrivals = myLeads.filter(l => l.status === 'visita_agendada' && l.checkInRequested);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5 font-mono flex items-center gap-2 flex-wrap">
                <span>{advertiser.email} • WhatsApp: {advertiser.phone}</span>
      ...
      <div className="flex border-b border-slate-200 mb-6 font-display font-medium text-sm">
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'funnel' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'produtos' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'financeiro' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
          Financeiro / Repasses
        </button>
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'afiliados' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
          Nossos Indicadores
        </button>
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'financiamentos' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
          Mesa de Financiamentos ({simulations?.filter(s => s.advertiserId === advertiser.id).length || 0})
        </button>
      </div>
        ...
        <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-600 mt-1">
                        O indicador <strong className="text-slate-900 font-bold">{lead.indicatorName}</strong> informou que acabou de chegar para a visita do cliente <strong className="text-slate-900 font-bold">{lead.clientName}</strong> (interesse em <span className="text-orange-700 font-semibold">{lead.productTitle}</span>).
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => {
                      ...
                      onAddNotification(`Presença do indicador ${lead.indicatorName} confirmada!`, 'success');
                    }}
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 text-xs text-amber-800 flex items-start gap-2.5">
            ...
            <div>
              ...
              <p className="mt-0.5 leading-snug">
                Neste painel você atua como o Lojista/Imobiliária. Você pode mover os leads gerados pelos indicadores através das fases de atendimento. Quando marcar como <strong>"Vendido"</strong>, você simulará o envio do contrato de venda para o comissionamento cair na carteira do indicador correspondente.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-start overflow-x-auto pb-4">
            {Object.entries(funnelStages).map(([stageId, stage]) => (
              <div key={stageId} className="bg-slate-50 border border-slate-150 rounded-2xl p-3 min-w-[190px] flex flex-col">
                ...
                <div className="space-y-3 min-h-[300px]">
                  {stage.list.map(lead => (
                    ...
                    >
                      <div className="flex justify-between items-start gap-1">
                        <span className="text-[9px] text-orange-600 font-bold block truncate uppercase flex-1">{lead.productTitle}</span>
                        <span className="text-[8px] bg-slate-100 text-slate-500 px-1 py-0.2 rounded font-semibold opacity-0 group-hover:opacity-100 transition-opacity">Abrir ↗</span>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-mono text-slate-400 mt-0.5">{lead.clientPhone}</p>
                      </div>
                      ...
                      {lead.referralChannel && (
                        <div className="flex items-center gap-1 text-[8px] font-bold uppercase tracking-wider text-slate-600 bg-slate-50 border border-slate-150 px-1.5 py-0.5 rounded w-max">
                          {lead.referralChannel.toLowerCase().includes('instagram') ? '📸' : 
                           ...
                           lead.referralChannel.toLowerCase().includes('tiktok') ? '🎵' : '🔗'} {lead.referralChannel}
                        </div>
                      )}
                      ...
                      {lead.visitDate && (
                        <div className="bg-amber-50 text-amber-950 border border-amber-200/60 px-2 py-1 rounded-lg text-[9px] font-semibold flex flex-col gap-0.5 font-sans">
                          ...
                          <span className="font-mono text-amber-900 pl-4">
                            {new Date(lead.visitDate).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}
                          </span>
                        </div>
                      )}
                      ...
                      <div className="border-t border-slate-100 pt-1.5 flex justify-between items-center text-[9px] text-slate-500 font-mono">
                        ...
                        <span className="font-bold text-orange-700">R$ {lead.commissionValue.toLocaleString('pt-BR')}</span>
                      </div>
                  ...
                  {stage.list.length === 0 && (
                    <p className="text-[10px] text-slate-400 text-center py-8 italic">Sem leads nesta fase</p>
                  )}
                </div>
              </div>
            ))}
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100/80">
                <p className="text-xs text-slate-600 leading-relaxed font-medium">
                  💡 <strong>IndicaAqui Dica:</strong> Bens com fotos de alta qualidade e descrições detalhadas recebem até 4x mais indicações da nossa rede. Defina comissões atraentes para incentivar os melhores indicadores.
                </p>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 font-sans text-slate-700">
                      {myProducts.map(p => (
                        <tr key={p.id} className="hover:bg-slate-50/50 transition-all">
                          <td className="py-4 px-4 flex items-center gap-3">
                            ...
                            />
                            <div>
                              <span className="text-[9px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wider font-mono">{p.category}</span>
                              <span className="block font-bold text-slate-900 leading-snug mt-1">{p.title}</span>
                            </div>
                          </td>
                          <td className="py-4 px-4 font-medium text-slate-600">{p.location.city} - {p.location.state}</td>
                          <td className="py-4 px-4 font-mono font-bold text-slate-900">R$ {p.price.toLocaleString('pt-BR')}</td>
                          <td className="py-4 px-4 font-mono font-semibold text-orange-600">
                            R$ {p.commissionDigitalValue?.toLocaleString('pt-BR')} ({p.commissionDigitalPct}%)
                          </td>
                          <td className="py-4 px-4 font-mono font-semibold text-emerald-600">
                            {p.allowPresencialTier ? (
                              <>R$ {p.commissionPresencialValue?.toLocaleString('pt-BR')} ({p.commissionPresencialPct}%)</>
                            ) : (
                          ...
                          <td className="py-4 px-4">
                            <span className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wide ${
                              p.status === 'ativo' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
                              p.status === 'reservado' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 
                      ...
                      {myProducts.length === 0 && (
                        <tr>
                          <td colSpan={7} className="text-center py-12 text-slate-400 italic font-medium bg-slate-50/20">
                            <div className="space-y-2">
                              <span className="text-3xl block">📦</span>
                              <p className="text-xs">Nenhum bem cadastrado no seu catálogo de indicações.</p>
                              <p className="text-[10px] text-slate-400">Cadastre um bem manualmente ou utilize a Carga em Lote para começar!</p>
                            </div>
                          </td>
                        </tr>
                      )}
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="font-display font-bold text-slate-900 text-sm">Carga em Lote de Anúncios</h4>
                    <p className="text-xs text-slate-500">Importe dezenas de anúncios de uma só vez utilizando arquivos CSV/Excel ou colando diretamente as células de sua planilha.</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between max-w-lg mx-auto bg-slate-50 p-1.5 rounded-xl border border-slate-100 text-center text-[10px] font-bold text-slate-400">
                  <div className={`flex-1 py-1.5 rounded-lg ${bulkStep === 'upload' ? 'bg-orange-600 text-white' : ''}`}>1. Arquivo ou Ctrl+V</div>
                  <div className="px-2 text-slate-300">➔</div>
                  <div className={`flex-1 py-1.5 rounded-lg ${bulkStep === 'mapping' ? 'bg-orange-600 text-white' : ''}`}>2. Mapeamento</div>
                  <div className="px-2 text-slate-300">➔</div>
                  <div className={`flex-1 py-1.5 rounded-lg ${bulkStep === 'validation' ? 'bg-orange-600 text-white' : ''}`}>3. Validação e Carga</div>
                </div>
                ...
                {bulkStep === 'upload' && (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <label className="block text-xs font-bold text-slate-700 uppercase">1. Selecione a Vertical de Destino</label>
                        <div className="grid grid-cols-5 gap-2">
                          {(['imovel', 'carro', 'moto', 'barco', 'jetski'] as const).map(cat => (
                            ...
                            >
                              ...
                              <span className="text-[8px] truncate block w-full text-center">{cat}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        <span className="block text-xs font-bold text-slate-700 uppercase">2. Baixar Modelo de Carga (CSV)</span>
                        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2">
                            <span className="text-2xl">📥</span>
                            <div className="text-left">
                              <span className="block text-xs font-bold text-slate-800">Modelo {bulkCategory.toUpperCase()}</span>
                              <span className="block text-[9px] text-slate-500">Colunas padrão para importação</span>
                            </div>
                          </div>
                          <button
                            type="button"
                            onClick={() => {
                              ...
                              const csvRows = bulkCategory === 'imovel' 
                              ...
                              onAddNotification("Modelo de planilha CSV baixado com sucesso!", "success");
                            }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* File Drag and Drop */}
                      <div className="space-y-2">
                        <label className="block text-xs font-bold text-slate-700 uppercase">Opção A: Carregar Arquivo de Dispositivo</label>
                        <div className="border-2 border-dashed border-slate-300 hover:border-orange-500 bg-slate-50 hover:bg-slate-100/40 rounded-2xl p-6 text-center transition-all relative">
                          ...
                          <div className="space-y-2 pointer-events-none">
                            <span className="text-3xl block">📄</span>
                            <span className="block text-xs font-bold text-slate-700">Arraste seu arquivo CSV ou clique para selecionar</span>
                            <span className="block text-[10px] text-slate-400 font-mono">Suporta delimitadores como vírgula (,), ponto e vírgula (;) ou Tabulações</span>
                      ...
                      {/* Copy-Paste area */}
                      <div className="space-y-2">
                        <label className="block text-xs font-bold text-slate-700 uppercase">Opção B: Copiar e Colar de sua Planilha (Excel / Sheets)</label>
                        <div className="space-y-1">
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] text-slate-400">Cole a linha de cabeçalhos e as linhas de dados juntas.</span>
                            <button
                              type="button"
                              onClick={() => {
                                const sample = "Título\tPreço\tDescrição\tCidade\tEstado\tImagemCapa\n" +
                                ...
                                onAddNotification("Modelo colado na área de texto! Clique em Processar.", "info");
                              }}
                  ...
                  <div className="space-y-4 animate-fade-in">
                    <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex gap-3">
                      ...
                      <div className="text-xs text-amber-800 leading-relaxed">
                        <strong>Mapeamento Inteligente:</strong> Identificamos os cabeçalhos de sua tabela. Associe cada campo do IndicaAqui com a respectiva coluna encontrada no seu arquivo ou texto colado.
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-150">
                      {Object.keys(bulkMapping).map((field) => (
                        <div key={field} className="space-y-1 bg-white p-3 rounded-xl border border-slate-200">
                          <div className="flex justify-between items-center">
                          ...
                          <select
                            ...
                            onChange={(e) => setBulkMapping({ ...bulkMapping, [field]: e.target.value })}
                ...
                {bulkStep === 'validation' && (
                  <div className="space-y-4 animate-fade-in">
                    <div className="flex justify-between items-center">
                      <h5 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Preview de Dados Mapeados e Validação ({bulkParsedRows.length} itens)</h5>
                      <span className="text-[10px] text-slate-400">Total detectado: {bulkParsedRows.length} linhas</span>
                    </div>
                        ...
                        <table className="w-full text-left text-xs">
                          ...
                          <tbody className="divide-y divide-slate-100 font-sans">
                            {bulkParsedRows.map((row, idx) => {
                              ...
                              const city = bulkMapping.city ? (row.data[bulkMapping.city] || 'São Paulo') : 'São Paulo';
                              const state = bulkMapping.state ? (row.data[bulkMapping.state] || 'SP') : 'SP';
                              const coverImage = bulkMapping.coverImage ? (row.data[bulkMapping.coverImage] || '') : '';
                              
                              const cleanPrice = parseFloat(priceRaw.replace(/[^\d.,]/g, '').replace(',', '.')) || 0;
                              const isValid = title && cleanPrice > 0;

                              return (
                                <tr key={row.id} className={isValid ? "hover:bg-slate-50/50" : "bg-red-50/30 hover:bg-red-50/50"}>
                                  <td className="py-3 px-4 font-mono text-[10px] font-bold text-slate-400">{idx + 1}</td>
                                  ...
                                  <td className="py-3 px-4 font-mono font-bold">
                                    {cleanPrice > 0 ? (
                                      ...
                                      <span className="text-red-500">Inválido ({priceRaw || 'Nulo'})</span>
                                    )}
                                  </td>
                                  <td className="py-3 px-4 font-medium text-slate-500">{city} - {state}</td>
                                  <td className="py-3 px-4">
                                    {coverImage ? (
                                      ...
                                      <span className="text-slate-400 italic text-[10px]">Imagem Fallback</span>
                                    )}
                                  </td>
                                  <td className="py-3 px-4">
                                    {isValid ? (
                                      <span className="bg-emerald-50 text-emerald-700 border border-emerald-100 font-bold text-[9px] px-2 py-0.5 rounded uppercase">Válido</span>
                                    ) : (
                                      <span className="bg-red-50 text-red-700 border border-red-100 font-bold text-[9px] px-2 py-0.5 rounded uppercase">Rejeitar</span>
                                    )}
                                  </td>
                                </tr>
                              );
                    ...
                    <div className="flex justify-between pt-4 border-t border-slate-100">
                      ...
                      >
                        Confirmar Importação de ({bulkParsedRows.filter(r => r.data[bulkMapping.title] && (parseFloat(r.data[bulkMapping.price].replace(/[^\d.,]/g, '').replace(',', '.')) || 0) > 0).length}) Itens <Check className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
                )}
              ...
              {!activeIntegrationDetail ? (
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-2">
                    <h4 className="font-display font-bold text-slate-900 text-sm">Central de Integrações e APIs</h4>
                    <p className="text-xs text-slate-500">Conecte o IndicaAqui diretamente com os sistemas que você já utiliza no seu negócio. Sincronize seu catálogo automaticamente e elimine o trabalho manual de cadastro.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:border-orange-500/50 transition-all group">
                      <div className="space-y-3">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="bg-slate-100 text-slate-600 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Imobiliário</span>
                        </div>
                        <div>
                          <h5 className="font-display font-bold text-slate-900 text-xs group-hover:text-orange-600 transition-colors">Vista CRM</h5>
                          <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Sincronize sua carteira de imóveis do Vista CRM diretamente para o IndicaAqui, atualizando preços e fotos diariamente.</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50 mt-4 flex justify-between items-center">
                        <span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${integrations.vistaCrm.active ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:border-orange-500/50 transition-all group">
                      <div className="space-y-3">
                        ...
                        <div>
                          <h5 className="font-display font-bold text-slate-900 text-xs group-hover:text-orange-600 transition-colors">Kenlo (inGaia)</h5>
                          <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Conexão direta com a plataforma imobiliária líder de mercado. Sincronização automatizada por meio de chaves API integradoras.</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50 mt-4 flex justify-between items-center">
                        <span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${integrations.kenlo.active ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:border-orange-500/50 transition-all group">
                      <div className="space-y-3">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="bg-slate-100 text-slate-600 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Automotivo</span>
                        </div>
                        <div>
                          <h5 className="font-display font-bold text-slate-900 text-xs group-hover:text-orange-600 transition-colors">Integradores de Veículos</h5>
                          <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Carregue seu estoque de carros, motos e náutica informando um Feed XML padrão Webmotors, iCarros, ou AutoGestor.</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50 mt-4 flex justify-between items-center">
                        <span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${integrations.webmotors.active ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:border-orange-500/50 transition-all group">
                      <div className="space-y-3">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="bg-slate-100 text-slate-600 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Varejo & Serviços</span>
                        </div>
                        <div>
                          <h5 className="font-display font-bold text-slate-900 text-xs group-hover:text-orange-600 transition-colors">Bling! ERP</h5>
                          <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Integração por API v3 do ERP de e-commerce mais utilizado no Brasil. Publique seu catálogo de produtos em lote.</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50 mt-4 flex justify-between items-center">
                        <span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${integrations.bling.active ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:border-orange-500/50 transition-all group">
                      <div className="space-y-3">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="bg-slate-100 text-slate-600 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">E-Commerce</span>
                        </div>
                        <div>
                          <h5 className="font-display font-bold text-slate-900 text-xs group-hover:text-orange-600 transition-colors">Shopify / WooCommerce</h5>
                          <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Conecte sua loja virtual diretamente. Importe bens de alto valor, acessórios e náutica usando tokens de aplicativos personalizados.</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50 mt-4 flex justify-between items-center">
                        <span className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${integrations.shopify.active ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                    ...
                    <div className="bg-slate-900 text-white p-5 rounded-3xl border border-slate-800 flex flex-col justify-between hover:bg-slate-950 transition-all group">
                      <div className="space-y-3">
                        <div className="flex justify-between items-start">
                          <span className="text-3xl">💻</span>
                          <span className="bg-orange-600/30 border border-orange-500/30 text-orange-300 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Para Desenvolvedores</span>
                        </div>
                        <div>
                          <h5 className="font-display font-bold text-white text-xs group-hover:text-orange-500 transition-colors">API Direta de Integração</h5>
                          <p className="text-[10px] text-slate-400 leading-relaxed mt-1">Desenvolveu seu próprio sistema? Integre seu backend via requisições HTTP REST (POST JSON) usando tokens de autenticação.</p>
                        </div>
                      </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6 animate-fade-in">
                  <div className="flex justify-between items-center border-b border-slate-150 pb-4">
                    ...
                    <span className="text-[10px] bg-orange-50 text-orange-700 border border-orange-100 px-3 py-1 rounded-full font-bold uppercase tracking-wider font-mono">
                      {activeIntegrationDetail === 'api_doc' ? 'API de Desenvolvedor' : `Integração: ${activeIntegrationDetail.toUpperCase()}`}
                    </span>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4 text-left">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl">🏢</span>
                          <div>
                            <h4 className="font-display font-bold text-slate-900 text-sm">Vista CRM Conector</h4>
                            <p className="text-[10px] text-slate-400 uppercase font-mono font-bold">Vertical: Imóveis / Lotes / Galpões</p>
                          </div>
                        </div>
                        <p className="text-xs text-slate-600 leading-relaxed">
                          O Vista CRM possui uma API de integração robusta. Insira seu token de API e subdomínio de cliente abaixo para realizar a autenticação e sincronizar imóveis em tempo real.
                        </p>
                      ...
                      <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-150 text-left">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">Subdomínio do Vista CRM (URL)</label>
                          <input
                            type="text"
                        ...
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">API Key / Token Vista</label>
                          <input
                            type="password"
                        ...
                        <div className="flex items-center gap-2 py-1">
                          ...
                          />
                          <label htmlFor="vista-auto-sync" className="text-xs text-slate-600 select-none font-medium">Sincronizar estoque automaticamente todos os dias às 03:00</label>
                        </div>

                        <div className="flex gap-2 pt-2">
                          <button
                            type="button"
                            onClick={() => onAddNotification("Conexão com Vista CRM estabelecida e autenticada! (Ping de API OK)", "success")}
                          ...
                          <button
                            ...
                            disabled={!integrations.vistaCrm.url || !integrations.vistaCrm.token || isSyncing}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4 text-left">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl">🏬</span>
                          <div>
                            <h4 className="font-display font-bold text-slate-900 text-sm">Kenlo (inGaia) Conector</h4>
                            <p className="text-[10px] text-slate-400 uppercase font-mono font-bold">Vertical: Imóveis / Lançamentos residenciais</p>
                          </div>
                        </div>
                        <p className="text-xs text-slate-600 leading-relaxed">
                          A Kenlo (antiga inGaia Imob) disponibiliza um sistema de integradores imobiliários via credenciais OAuth e chave da agência. Preencha seus tokens de produção de agência para integrar seu portfólio.
                        </p>
                      </div>
                      ...
                      <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-150 text-left">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="block text-[10px] font-bold text-slate-700 uppercase">Client ID</label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1">
                            <label className="block text-[10px] font-bold text-slate-700 uppercase">Agência ID</label>
                            <input
                              type="text"
                        ...
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">Client Secret (Token)</label>
                          <input
                            type="password"
                        ...
                        <div className="flex gap-2 pt-2">
                          <button
                            type="button"
                            onClick={() => onAddNotification("Kenlo API autenticada com sucesso! Credenciais salvas.", "success")}
                          ...
                          <button
                            ...
                            disabled={!integrations.kenlo.clientId || !integrations.kenlo.clientSecret || isSyncing}
                            className={`flex-1 font-bold text-[10px] py-2 px-4 rounded-xl text-center transition-all flex items-center justify-center gap-1.5 shadow ${
                              integrations.kenlo.clientId && integrations.kenlo.clientSecret && !isSyncing
                                ? 'bg-orange-600 text-white hover:bg-orange-500 cursor-pointer'
                                : 'bg-slate-100 text-slate-400 cursor-not-allowed'
                            }`}
                          >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4 text-left">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl">🚗</span>
                          <div>
                            <h4 className="font-display font-bold text-slate-900 text-sm">XML Feed Integrador (Webmotors / iCarros)</h4>
                            <p className="text-[10px] text-slate-400 uppercase font-mono font-bold">Vertical: Veículos / Motos / Náutica</p>
                          </div>
                        </div>
                        <p className="text-xs text-slate-600 leading-relaxed">
                          Importação contínua de estoque por meio de URLs de Feed XML estruturados. Nosso sistema lê o padrão oficial Webmotors 2.0 de metadados de veículos (Ano, KM, Cor, Opcionais, etc.) and o traduz em anúncios mapeados para indicação.
                        </p>
                      </div>

                      <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-150 text-left">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">URL do XML Feed de Estoque</label>
                          <input
                            type="text"
                        ...
                        <div className="flex gap-2 pt-2">
                          <button
                            type="button"
                            onClick={() => onAddNotification("URL de Feed XML validada com sucesso! 2 veículos identificados.", "success")}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4 text-left">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl">📦</span>
                          <div>
                            <h4 className="font-display font-bold text-slate-900 text-sm">Bling! ERP API v3</h4>
                            <p className="text-[10px] text-slate-400 uppercase font-mono font-bold">Vertical: Geral / Serviços / Cotas de Alto Valor</p>
                          </div>
                        </div>
                        <p className="text-xs text-slate-600 leading-relaxed">
                          Conecte o ERP Bling! utilizando as credenciais de API v3 baseadas em OAuth2 ou Chave de Acesso de Aplicativo. Publique produtos do seu almoxarifado direto para indicações, ideal para revendas de veículos ou empresas de serviços especiais.
                        </p>
                      </div>

                      <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-150 text-left">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">Chave API v3 Bling!</label>
                          <input
                            type="password"
                        ...
                        <div className="flex gap-2 pt-2">
                          <button
                            type="button"
                            onClick={() => onAddNotification("API Token Bling! validado com o servidor do ERP.", "success")}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4 text-left">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h4 className="font-display font-bold text-slate-900 text-sm">Shopify / WooCommerce Sync</h4>
                            <p className="text-[10px] text-slate-400 uppercase font-mono font-bold">Vertical: Bens E-Commerce / Náutica e Luxo</p>
                          </div>
                        </div>
                        <p className="text-xs text-slate-600 leading-relaxed">
                          Sincronize lojas virtuais do Shopify ou WooCommerce por meio de APIs REST. Informe as credenciais do Custom App geradas no seu Admin do Shopify para sincronização instantânea.
                        </p>
                      </div>

                      <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-150 text-left">
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">Domínio da Loja (.myshopify.com)</label>
                          <input
                            type="text"
                        ...
                        <div className="space-y-1">
                          <label className="block text-[10px] font-bold text-slate-700 uppercase">Access Token (Admin API)</label>
                          <input
                            type="password"
                            placeholder="shpat_..."
                        ...
                        <div className="flex gap-2 pt-2">
                          <button
                            type="button"
                            onClick={() => onAddNotification("Tokens de Custom App Shopify aceitos com sucesso!", "success")}
                            disabled={!integrations.shopify.shopUrl || !integrations.shopify.accessToken}
                            className={`flex-1 font-bold text-[10px] py-2 px-4 rounded-xl border text-center transition-all ${
                          ...
                          <button
                            ...
                            disabled={!integrations.shopify.shopUrl || !integrations.shopify.accessToken || isSyncing}
                            className={`flex-1 font-bold text-[10px] py-2 px-4 rounded-xl text-center transition-all flex items-center justify-center gap-1.5 shadow ${
                              integrations.shopify.shopUrl && integrations.shopify.accessToken && !isSyncing
                                ? 'bg-orange-600 text-white hover:bg-orange-500 cursor-pointer'
                                : 'bg-slate-100 text-slate-400 cursor-not-allowed'
                            }`}
                          >
                  ...
                  {activeIntegrationDetail === 'api_doc' && (
                    <div className="space-y-4 text-left">
                      <div className="flex items-center gap-2">
                        <span className="text-2xl">💻</span>
                        <div>
                          <h4 className="font-display font-bold text-slate-900 text-sm">Integração Direta por API REST</h4>
                          <p className="text-xs text-slate-500">Ideal para sistemas legados ou desenvolvimento sob medida. Seu desenvolvedor pode cadastrar produtos na plataforma em tempo real.</p>
                        </div>
                      </div>

                      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-4">
                        <div className="space-y-1">
                          <span className="block text-[10px] font-bold text-slate-500 uppercase">Seu Token Privado de API (Bearer Token)</span>
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                onAddNotification("Token copiado para a área de transferência!", "success");
                              }}
                          ...
                          <span className="block text-[9px] text-red-500 font-bold">Atenção: Não compartilhe esse token publicamente. Ele dá acesso de escrita ao seu catálogo.</span>
                        </div>

                        <div className="space-y-2">
                          <span className="block text-[10px] font-bold text-slate-500 uppercase">Instrução cURL de Exemplo (Adicionar Produto)</span>
    ...
    "location": {
      ...
      "state": "SP"
                        ...
                        <div className="bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-600 space-y-2">
                          ...
                          <p>• <span className="font-mono font-bold text-orange-600">201 Created</span>: Produto criado com sucesso.</p>
                          <p>• <span className="font-mono font-bold text-red-600">401 Unauthorized</span>: Token ausente ou inválido.</p>
                          <p>• <span className="font-mono font-bold text-red-600">422 Unprocessable Entity</span>: Falha de validação dos atributos requeridos.</p>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      {activeTab === 'financeiro' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm lg:col-span-1 space-y-4">
            <h3 className="font-display font-bold text-slate-800 text-base">Fatura / Mensalidade</h3>
            ...
            <div className="bg-gradient-to-br from-orange-950 to-slate-950 text-white rounded-2xl p-5 border border-orange-900/30 relative overflow-hidden">
              <span className="text-[9px] bg-orange-600 text-white px-2.5 py-0.5 rounded font-bold uppercase tracking-wider">PREMIUM</span>
              <h4 className="font-display font-bold text-lg mt-2">Plano Corporativo</h4>
              <p className="text-xs text-slate-300 mt-1">Limite de anúncios ilimitado • Acesso a todas as verticais.</p>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm lg:col-span-2 space-y-4">
            <h3 className="font-display font-bold text-slate-800 text-base">Comissões Pagas ou Pendentes</h3>
            <p className="text-xs text-slate-500">Auditoria completa de repasses devidos aos indicadores que trouxeram vendas.</p>

            <div className="space-y-3">
              {myLeads.filter(l => l.status === 'venda_concluida').map(lead => (
                <div key={lead.id} className="flex justify-between items-center border border-slate-100 rounded-2xl p-4 bg-slate-50">
                  <div className="flex gap-3">
                    <div className="bg-emerald-100 text-emerald-800 w-10 h-10 rounded-xl flex items-center justify-center font-bold">✓</div>
                    <div>
                      <h4 className="font-bold text-sm text-slate-950">Comissão Quitada</h4>
                      <p className="text-xs text-slate-500">{lead.productTitle} • Recebido por: {lead.indicatorName}</p>
                    </div>
                  </div>
                  <div className="text-right font-mono">
                    <span className="text-sm font-bold text-emerald-600 block">R$ {lead.commissionValue.toLocaleString('pt-BR')}</span>
                    <span className="text-[9px] text-slate-400 uppercase font-bold block">Ref: {lead.commissionType}</span>
                  </div>
                </div>
              ))}
              {myLeads.filter(l => l.status === 'proposta').map(lead => (
                <div key={lead.id} className="flex justify-between items-center border border-slate-150 rounded-2xl p-4 bg-amber-50">
                  <div className="flex gap-3">
                    <div className="bg-amber-100 text-amber-800 w-10 h-10 rounded-xl flex items-center justify-center font-bold">🕒</div>
                    <div>
                      <h4 className="font-bold text-sm text-slate-950">Aguardando Faturamento</h4>
                      <p className="text-xs text-slate-500">{lead.productTitle} • Negociação de proposta por: {lead.indicatorName}</p>
                    </div>
                  </div>
                  <div className="text-right font-mono">
                    <span className="text-sm font-bold text-amber-600 block">R$ {lead.commissionValue.toLocaleString('pt-BR')}</span>
                    <button
              ...
              {myLeads.length === 0 && (
                <p className="text-xs text-slate-400 text-center py-8">Sem movimentação de comissão relevante</p>
              )}
            </div>
          </div>
        </div>
      ...
      {activeTab === 'afiliados' && (
        <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
          <h3 className="font-display font-bold text-slate-800 text-base">Rede de Indicadores Ativos</h3>
          <p className="text-xs text-slate-500">Métricas de performance dos promotores que estão divulgando sua loja.</p>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {indicators.filter(i => i.name).map(ind => (
              <div key={ind.id} className="border border-slate-100 rounded-2xl p-4 hover:border-slate-200 transition-all space-y-3 bg-slate-50/50">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-wider ${
                    ind.league === 'ouro' ? 'bg-amber-100 text-amber-800' : 'bg-slate-200 text-slate-700'
                  }`}>
                    {ind.league}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-center text-[10px] font-mono border-t border-slate-100 pt-2">
                  <div>
                    <span className="text-[9px] text-slate-400 uppercase font-bold block">Cliques Gerados</span>
                    <span className="font-bold text-slate-800 text-xs">{ind.clicks}</span>
                  </div>
                  <div className="border-l border-slate-200">
                    <span className="text-[9px] text-slate-400 uppercase font-bold block">Reputação Score</span>
                ...
                <div className="pt-2 flex gap-1">
                  ...
                  <button
                    onClick={() => {
                      onAddNotification(`Indicador ${ind.name} foi notificado para revisão de dados.`, 'info');
                    }}
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="font-display font-bold text-slate-900 text-xl mt-2">Escolha a Vertical e Cadastre o Bem</h2>
              <p className="text-xs text-slate-500 mt-1">O formulário de atributos de dados se adaptará automaticamente.</p>
            </div>
            ...
            <form onSubmit={handleAddProductSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Título do Anúncio</label>
                  <input 
                    type="text" required
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Descrição Comercial</label>
                  <textarea 
                    required rows={2}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Preço de Venda (R$)</label>
                  <input 
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Cidade</label>
                    <input 
                      type="text" required
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={e => setProductForm({...productForm, state: e.target.value.toUpperCase()})}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-xs focus:outline-none"
                    />
                  </div>
                </div>
              ...
              {/* DYNAMIC ATTRIBUTES FIELDS BASED ON CATEGORY (JSONB Flex schema simulation) */}
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <span className="block text-xs font-bold text-slate-800 uppercase mb-3">Atributos Técnicos Específicos ({newProductCategory.toUpperCase()})</span>
                
                {newProductCategory === 'imovel' && (
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold uppercase">Tipo</label>
                      <select 
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, type: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs"
                      >
                        <option value="Apartamento">Apartamento</option>
                        <option value="Casa de Condomínio">Casa Condomínio</option>
                        <option value="Cobertura">Cobertura</option>
                        <option value="Terreno">Terreno</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Área Útil (m²)</label>
                      <input 
                        type="number" placeholder="120"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, areaUseful: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Suítes</label>
                      <input 
                        type="number" placeholder="3"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, suites: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Taxa Condomínio (R$)</label>
                      <input 
                        type="number" placeholder="1200"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, condoFee: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Número da Matrícula / Registro</label>
                      <input 
                        type="text" placeholder="MT-1120"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, registryId: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Vagas Garagem</label>
                      <input 
                        type="number" placeholder="2"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, parkingSpaces: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                  </div>
                ...
                {newProductCategory === 'carro' && (
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold uppercase">Marca</label>
                      <input 
                        type="text" placeholder="Porsche / BMW" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, brand: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">KM Rodados</label>
                      <input 
                        type="number" placeholder="12000" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, km: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Cor Externa</label>
                      <input 
                        type="text" placeholder="Cinza Giz"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, color: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Câmbio</label>
                      <input 
                        type="text" placeholder="PDK"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, transmission: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Placa (Uso Interno)</label>
                      <input 
                        type="text" placeholder="E*A-9E11" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, plate: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Laudo Cautelar Aprovado?</label>
                      <select 
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, cautionaryReport: e.target.value === 'sim'}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      >
                        <option value="sim">Sim (Dekra/Supervisão)</option>
                ...
                {newProductCategory === 'moto' && (
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Marca</label>
                      <input 
                        type="text" placeholder="Ducati / Harley" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, brand: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Cilindradas (cc)</label>
                      <input 
                        type="number" placeholder="1103" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, cc: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">KM</label>
                      <input 
                ...
                {newProductCategory === 'barco' && (
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Estaleiro (Fabricante)</label>
                      <input 
                        type="text" placeholder="NX Boats" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, builder: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Tamanho (Pés)</label>
                      <input 
                        type="number" placeholder="36" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, lengthFeet: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Horas de Motor</label>
                      <input 
                        type="number" placeholder="140" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, engineHours: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                  </div>
                ...
                {newProductCategory === 'jetski' && (
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Marca</label>
                      <input 
                        type="text" placeholder="Sea-Doo / Yamaha" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, brand: e.target.value}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Horas de Uso</label>
                      <input 
                        type="number" placeholder="32" required
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, hours: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-1 font-semibold">Capacidade (Pessoas)</label>
                      <input 
                        type="number" placeholder="3"
                        onChange={e => setProductForm({...productForm, attributes: {...productForm.attributes, passengerCapacity: Number(e.target.value)}})}
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1"
                      />
                    </div>
                  </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <span className="block text-xs font-bold text-slate-800 uppercase mb-1">Configuração de Comissionamento</span>
                  <p className="text-[10px] text-slate-400">Insira a porcentagem de repasse sobre o valor de venda.</p>
                </div>

                <div>
                  <label className="block text-[10px] text-slate-600 font-semibold mb-1 uppercase">Digital (%)</label>
                  <input 
                    type="number" step="0.1" required
                    value={productForm.commissionDigitalPct}
                    onChange={e => setProductForm({...productForm, commissionDigitalPct: e.target.value})}
                    placeholder="1"
                ...
                <div>
                  <label className="block text-[10px] text-slate-600 font-semibold mb-1 uppercase flex items-center justify-between">
                    ...
                    <input 
                      ...
                      onChange={e => setProductForm({...productForm, allowPresencialTier: e.target.checked})}
                  ...
                  <input 
                    ...
                    onChange={e => setProductForm({...productForm, commissionPresencialPct: e.target.value})}
                    placeholder="3"
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                  <span className="block text-xs font-semibold text-slate-700 uppercase">Imagens do Anúncio</span>
                  <div className="flex bg-slate-100 p-0.5 rounded-lg text-[10px] font-bold uppercase">
                    <span className="px-3 py-1 rounded-md bg-white text-slate-800 shadow">
                      {uploadedImages.length > 0 ? 'Upload do Dispositivo Ativo' : 'Preset ou Upload'}
                    </span>
                  </div>
                </div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      Promise.all(readers).then(results => {
                        ...
                        onAddNotification(`${results.length} imagem(ns) carregada(s) do seu dispositivo!`, 'success');
                      });
                    }}
                  />
                  <div className="space-y-1 pointer-events-none">
                    <div className="text-2xl">📤</div>
                    <span className="block text-xs font-bold text-slate-700">Clique ou arraste para carregar fotos do dispositivo</span>
                    <span className="block text-[10px] text-slate-400">Você pode carregar várias imagens (galeria completa)</span>
                  </div>
                </div>
                ...
                {uploadedImages.length > 0 ? (
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-slate-400 uppercase font-bold">Galeria Carregada ({uploadedImages.length} fotos)</span>
                      <button
                        type="button"
                        onClick={() => {
                          setUploadedImages([]);
                  ...
                  <div className="space-y-2">
                    <span className="block text-[10px] text-slate-400 uppercase font-bold">Ou selecione uma foto de capa rápida das predefinidas:</span>
                    <div className="grid grid-cols-3 gap-3">
                      {presetImages[newProductCategory].map((img, idx) => (
                        <div 
                          ...
                          className={`relative rounded-xl overflow-hidden cursor-pointer border-2 aspect-video transition-all ${
                            productForm.coverImage === img ? 'border-orange-600 scale-[1.03] shadow-md' : 'border-transparent'
                          }`}
                        >
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="font-display font-bold text-slate-900 text-lg">Faturamento & Quitação de Venda</h2>
              <p className="text-xs text-slate-500 mt-1">Garante a auditoria correta do faturamento da venda.</p>
            </div>
            ...
            <form onSubmit={handleCloseSaleSubmit} className="space-y-4">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-xs text-slate-700 space-y-2.5">
                <div>
                  ...
                  <span className="text-sm font-bold text-slate-900">{closingSaleLead.clientName}</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-400 block uppercase font-bold">Indicado por</span>
                  <span className="text-xs font-semibold text-slate-900">{closingSaleLead.indicatorName}</span>
                </div>
                <div className="grid grid-cols-2 gap-2 border-t border-slate-150 pt-2.5">
                  <div>
                    <span className="text-[9px] text-slate-400 block uppercase font-bold">Comissão Devida</span>
                    <span className="text-sm font-bold text-emerald-600">R$ {closingSaleLead.commissionValue.toLocaleString('pt-BR')}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-400 block uppercase font-bold">Atribuição</span>
                    <span className="text-xs font-bold uppercase text-orange-700">{closingSaleLead.commissionType}</span>
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Notas Internas de Fechamento</label>
                <textarea 
                  rows={2} required
                  value={saleNotes}
              ...
              <div className="border-2 border-dashed border-slate-200 hover:border-orange-500 rounded-2xl p-5 text-center transition-colors cursor-pointer bg-slate-50/50">
                <input 
                  type="file" id="invoice_file_upload" accept="image/*,.pdf"
                  onChange={() => {
                    setInvoiceUploaded(true);
                    onAddNotification('Nota Fiscal/Contrato anexado ao relatório de fechamento!', 'info');
                  }}
                  className="hidden"
                />
                <label htmlFor="invoice_file_upload" className="cursor-pointer space-y-1 block">
                  <Upload className="w-6 h-6 text-slate-400 mx-auto" />
                  <span className="block text-xs font-bold text-orange-600">Anexar NF-e ou Contrato de Compra e Venda</span>
                  <span className="block text-[9px] text-slate-400">PDF ou Imagem de comprovação de fechamento</span>
                </label>
              </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-2xl leading-relaxed">
                Gerencie as fichas cadastrais enviadas pelos seus Indicadores Parceiros. 
                Consulte as mesas de crédito dos bancos integrados, simule as parcelas e retorne as opções aprovadas. 
                O indicador poderá intermediar diretamente com o cliente para a assinatura digital do contrato!
              </p>
            </div>
            <span className="bg-orange-950/60 border border-orange-800 text-orange-400 text-[10px] font-mono px-3 py-1 rounded-full uppercase tracking-wider shrink-0">
              Mesa Ativa • {simulations?.filter(s => s.advertiserId === advertiser.id).length || 0} Fichas
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="bg-white border border-slate-100 p-4 rounded-2xl text-center">
              <span className="block text-[10px] text-slate-400 font-bold uppercase">Aguardando Loja</span>
              <span className="block font-display font-bold text-lg text-amber-500 mt-1">
                {simulations?.filter(s => s.advertiserId === advertiser.id && s.status === 'pendente').length || 0}
              </span>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-2xl text-center">
              ...
              <span className="block font-display font-bold text-lg text-indigo-500 mt-1">
                {simulations?.filter(s => s.advertiserId === advertiser.id && s.status === 'analise_bancos').length || 0}
              </span>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-2xl text-center">
              ...
              <span className="block font-display font-bold text-lg text-emerald-500 mt-1">
                {simulations?.filter(s => s.advertiserId === advertiser.id && s.status === 'aprovado').length || 0}
              </span>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-2xl text-center">
              <span className="block text-[10px] text-slate-400 font-bold uppercase">Contratos Assinados</span>
              <span className="block font-display font-bold text-lg text-slate-900 mt-1">
                {simulations?.filter(s => s.advertiserId === advertiser.id && s.status === 'concluido').length || 0}
              </span>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-2xl text-center col-span-2 md:col-span-1">
              <span className="block text-[10px] text-slate-400 font-bold uppercase">Crédito Recusado</span>
              <span className="block font-display font-bold text-lg text-red-500 mt-1">
                {simulations?.filter(s => s.advertiserId === advertiser.id && s.status === 'rejeitado').length || 0}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-xs">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h4 className="font-display font-bold text-slate-900 text-sm">Fichas de Crédito Recebidas</h4>
              <p className="text-[11px] text-slate-500">Ordene por data para priorizar o atendimento rápido</p>
            </div>

            <div className="divide-y divide-slate-100">
              {simulations?.filter(s => s.advertiserId === advertiser.id).map(sim => (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="lg:col-span-3 space-y-2">
                      <div className="flex gap-3 items-center">
                        <div className="w-12 h-9 bg-slate-100 rounded-lg overflow-hidden border border-slate-200 shrink-0">
                          <img 
                            src={sim.productImage || 'https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?auto=format&fit=crop&w=300&q=80'} 
                        ...
                        <div>
                          <h5 className="font-bold text-xs text-slate-900 leading-tight">{sim.productTitle}</h5>
                          <span className="text-[10px] text-slate-500 font-mono">Valor: R$ {sim.productPrice.toLocaleString('pt-BR')}</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-100/60 p-2.5 rounded-xl border border-slate-200/50 text-[10px] text-slate-600 space-y-0.5">
                        ...
                        <p className="font-bold text-orange-700 line-clamp-1">{sim.indicatorName}</p>
                        <p className="text-[9px] text-slate-400 font-mono">ID: {sim.id.substring(0, 8).toUpperCase()}</p>
                      </div>
                    </div>
                    ...
                    <div className="lg:col-span-4 bg-slate-50 p-3 rounded-2xl border border-slate-100 text-[11px] leading-relaxed text-slate-700 grid grid-cols-2 gap-x-3 gap-y-1">
                      <div className="col-span-2 pb-1 border-b border-slate-200/50 mb-1 flex items-center gap-1.5">
                        ...
                        <span className="font-bold text-[10px] text-slate-800 uppercase tracking-wider">Cadastro do Comprador</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block font-medium">Nome:</span>
                        <span className="font-bold text-slate-900 line-clamp-1">{sim.clientName}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-900 font-mono">{sim.clientBirthDate}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block font-medium">Renda Mensal:</span>
                        <span className="font-bold text-emerald-700 font-mono">R$ {sim.clientIncome.toLocaleString('pt-BR')}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block font-medium">Entrada Proposta:</span>
                        <span className="font-bold text-slate-900 font-mono">R$ {sim.downPayment.toLocaleString('pt-BR')}</span>
                      </div>
                    </div>
                    ...
                    <div className="lg:col-span-3 space-y-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        {sim.status === 'pendente' && (
                          <span className="bg-amber-100 text-amber-800 font-bold text-[9px] px-2 py-0.5 rounded-full uppercase">Pendente</span>
                        )}
                        {sim.status === 'analise_bancos' && (
                          <span className="bg-indigo-100 text-indigo-800 font-bold text-[9px] px-2 py-0.5 rounded-full uppercase">Em Análise</span>
                        )}
                        {sim.status === 'aprovado' && (
                          <span className="bg-emerald-100 text-emerald-800 font-bold text-[9px] px-2 py-0.5 rounded-full uppercase">Crédito Aprovado</span>
                        )}
                        {sim.status === 'rejeitado' && (
                          <span className="bg-red-100 text-red-800 font-bold text-[9px] px-2 py-0.5 rounded-full uppercase">Recusado</span>
                        )}
                        {sim.status === 'concluido' && (
                          <span className="bg-slate-950 text-slate-100 font-bold text-[9px] px-2 py-0.5 rounded-full uppercase">Assinado</span>
                        )}
                      </div>
                      ...
                      {sim.approvedContract ? (
                        <div className="bg-slate-900 text-white rounded-xl p-3 border border-slate-800 space-y-1 text-[10px] leading-relaxed">
                          <p className="font-bold text-[9px] text-emerald-400 uppercase tracking-wider flex items-center gap-1">
                            <Check className="w-3 h-3 text-emerald-400" /> {sim.approvedContract.bankName}
                          </p>
                          <p className="font-mono text-slate-300">
                            Termo: <strong className="font-bold text-white">{sim.approvedContract.installmentsCount}x R$ {sim.approvedContract.installmentValue.toLocaleString('pt-BR')}</strong>
                          </p>
                          <p className="font-mono text-slate-400">
                            Taxa: <span className="font-bold text-orange-400">{sim.approvedContract.interestRate}% a.m.</span> • Financiado: R$ {sim.approvedContract.approvedAmount.toLocaleString('pt-BR')}
                          </p>
                        </div>
                      ) : (
              ...
              {simulations?.filter(s => s.advertiserId === advertiser.id).length === 0 && (
                <div className="text-center py-20 bg-white">
                  ...
                  <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 leading-relaxed">
                    Você ainda não recebeu propostas de financiamento dos indicadores cadastrados para os seus anúncios de veículos.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 relative font-sans shadow-2xl max-h-[95vh] overflow-y-auto">
            ...
            <div className="text-center mb-6 border-b border-slate-100 pb-4">
              ...
              <h2 className="font-display font-bold text-slate-900 text-xl mt-2">Simular e Aprovar Financiamento</h2>
              <p className="text-xs text-slate-500 mt-1">
                Configure os bancos credenciados e registre os termos do contrato aprovado de financiamento.
              </p>
            </div>
            ...
            <form onSubmit={handleSaveSimUpdates} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {editBankResponses.map((bank, index) => (
                    <div key={index} className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-slate-800 line-clamp-1">{bank.bankName}</span>
                        <span className="bg-orange-150 text-orange-800 text-[8px] font-bold px-1 py-0.5 rounded">Banco</span>
                      </div>

                      <div className="space-y-1">
                        <div>
                          <label className="text-[9px] text-slate-400 block font-semibold">Valor Financiado</label>
                          <input 
                            type="number" required
                        ...
                        <div>
                          <label className="text-[9px] text-slate-400 block font-semibold">Parcela (R$)</label>
                          <input 
                            type="number" required
                        ...
                        <div>
                          <label className="text-[9px] text-slate-400 block font-semibold">Taxa de Juros (% a.m.)</label>
                          <input 
              ...
              <div className="border-t border-slate-100 pt-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Instituição de Crédito Escolhida</label>
                    <select 
                      value={approvedContractForm.bankName}
                      onChange={e => setApprovedContractForm({...approvedContractForm, bankName: e.target.value})}
                    ...
                    >
                      ...
                      <option value="Banco Bradesco Financiamentos">Banco Bradesco Financiamentos</option>
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Valor de Financiamento Aprovado (R$)</label>
                    <input 
                      type="number" required
                      value={approvedContractForm.approvedAmount}
                      onChange={e => setApprovedContractForm({...approvedContractForm, approvedAmount: e.target.value})}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 font-mono"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Valor de Entrada Requerida (R$)</label>
                    <input 
                      type="number" required
                      value={approvedContractForm.downPaymentRequired}
                      onChange={e => setApprovedContractForm({...approvedContractForm, downPaymentRequired: e.target.value})}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 font-mono"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Taxa de Juros Cadastrada (% a.m.)</label>
                    <input 
                      type="number" step="0.01" required
                      value={approvedContractForm.interestRate}
                      onChange={e => setApprovedContractForm({...approvedContractForm, interestRate: e.target.value})}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Quantidade de Parcelas</label>
                    <select 
                      value={approvedContractForm.installmentsCount}
                      onChange={e => setApprovedContractForm({...approvedContractForm, installmentsCount: parseInt(e.target.value)})}
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Valor Unitário da Parcela (R$)</label>
                    <input 
                      type="number" required
                      value={approvedContractForm.installmentValue}
                      onChange={e => setApprovedContractForm({...approvedContractForm, installmentValue: e.target.value})}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 font-mono"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-700 mb-1">Instruções para o Indicador / Comprador</label>
                  <textarea 
                    rows={2} required
                    value={approvedContractForm.additionalNotes}
                    onChange={e => setApprovedContractForm({...approvedContractForm, additionalNotes: e.target.value})}
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="font-display font-bold text-slate-900 text-lg">Notificar Indicador via WhatsApp</h2>
              <p className="text-xs text-slate-500 mt-1">
                Uma mensagem informando sobre a conclusão da análise de financiamento e o link do painel do indicador foi gerada com sucesso!
              </p>
            </div>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const phoneWithCountry = cleanedPhone.length === 10 || cleanedPhone.length === 11 ? '55' + cleanedPhone : cleanedPhone;
      ...
      {viewingLead && (() => {
        ...
        if (viewingLead.referralChannel?.toLowerCase().includes("instagram")) {
          channelDesc = "Este lead foi gerado após clicar em um link compartilhado no perfil (Bio) ou Stories do Instagram do indicador.";
        } else if (viewingLead.referralChannel?.toLowerCase().includes("whatsapp")) {
          channelDesc = "Este lead foi gerado através de um link de indicação enviado diretamente pelo WhatsApp pelo indicador.";
        } else if (viewingLead.referralChannel?.toLowerCase().includes("facebook")) {
          channelDesc = "Este lead veio de uma postagem ou indicação em grupos do Facebook do indicador.";
          ...
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="bg-white rounded-3xl max-w-2xl w-full p-6 relative font-sans shadow-2xl my-8">
              ...
              <p className="text-xs text-slate-500 mb-6 border-b border-slate-100 pb-4">
                Consulte todas as informações do lead de venda e do indicador autônomo responsável pelo direcionamento.
              </p>
                ...
                <div className="space-y-4">
                  <div className="bg-orange-50/50 p-4 rounded-2xl border border-orange-100/40">
                    ...
                    <div className="space-y-2 text-xs">
                      <div>
                        <span className="text-slate-400 block font-semibold uppercase text-[9px]">Nome do Comprador</span>
                        <p className="text-slate-900 font-bold text-sm">{viewingLead.clientName}</p>
                      </div>
                      <div>
                        <span className="text-slate-400 block font-semibold uppercase text-[9px]">Telefone de Contato</span>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-slate-900 font-mono font-semibold">{viewingLead.clientPhone}</span>
                          <a 
                      ...
                      {viewingLead.clientEmail && (
                        <div>
                          <span className="text-slate-400 block font-semibold uppercase text-[9px]">E-mail</span>
                          <p className="text-slate-900 font-mono font-medium">{viewingLead.clientEmail}</p>
                        </div>
                      )}
                      <div>
                        <span className="text-slate-400 block font-semibold uppercase text-[9px]">Data de Entrada</span>
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-150 text-xs">
                    ...
                    <div className="space-y-1">
                      <p className="font-bold text-slate-950 text-sm">{viewingLead.productTitle}</p>
                      <p className="text-[10px] text-slate-500 capitalize">Categoria: {viewingLead.productCategory}</p>
                      {relativeProduct && (
                      ...
                      <p className="text-[11px] font-bold text-emerald-600">
                        Comissão do Indicador: R$ {viewingLead.commissionValue.toLocaleString('pt-BR')}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-150 text-xs">
                    ...
                    {associatedInd ? (
                      <div className="space-y-2">
                        <div>
                          <span className="text-slate-400 block font-semibold uppercase text-[9px]">Indicador Responsável</span>
                          <p className="text-slate-900 font-bold text-sm">{associatedInd.name}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mt-1">
                          <div>
                            <span className="text-slate-400 block uppercase text-[8px] font-bold">Liga Atual</span>
                            <p className="text-slate-800 font-bold capitalize text-[11px] flex items-center gap-1">
                              👑 {associatedInd.league}
                            </p>
                          </div>
                          <div>
                            <span className="text-slate-400 block uppercase text-[8px] font-bold">Pontuação</span>
                        ...
                        <div className="border-t border-slate-200/60 pt-2 mt-2 space-y-1">
                          <p className="text-[10px] text-slate-600 truncate"><strong>E-mail:</strong> {associatedInd.email}</p>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] text-slate-400">Indicador importado por link direto de simulação.</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-6 border-t border-slate-100 pt-5">
                <div className="bg-amber-50/40 border border-amber-100 p-4 rounded-2xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <div>
                      <label className="block text-[11px] font-semibold text-slate-700 mb-1">Dia da Visita</label>
                      <input 
                        type="date"
                        defaultValue={viewingLead.visitDate ? viewingLead.visitDate.substring(0, 10) : ''}
                    ...
                    <div>
                      <label className="block text-[11px] font-semibold text-slate-700 mb-1">Horário da Visita</label>
                      <input 
                        type="time"
                        defaultValue={viewingLead.visitDate ? viewingLead.visitDate.substring(11, 16) : ''}
                  ...
                  <div className="mt-3 text-xs">
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Observações do Atendimento / Histórico de Contato</label>
                    <textarea 
                      rows={2}
                  ...
                  <div className="flex justify-end mt-4">
                    <button
                      type="button"
                      onClick={() => {
                        const dateVal = (document.getElementById('details_visit_date') as HTMLInputElement)?.value;
                        const timeVal = (document.getElementById('details_visit_time') as HTMLInputElement)?.value;
                        const notesVal = (document.getElementById('details_visit_notes') as HTMLTextAreaElement)?.value;
                        ...
                        onUpdateLeadStatus(
                          viewingLead.id, 
                          viewingLead.visitDate || combinedDate ? 'visita_agendada' : viewingLead.status, 
                          { 
                        ...
                        onAddNotification('Dados de agendamento e observações salvos com sucesso!', 'success');
                        setViewingLead(null);
                      }}
              ...
              <div className="mt-6 border-t border-slate-100 pt-5 text-left">
                ...
                <div className="bg-orange-50 border border-orange-100 p-3 rounded-2xl text-[10px] text-orange-950 mb-3 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5">
                      Para sua segurança e do indicador <strong>{viewingLead.indicatorName}</strong>, este canal de chat possui filtros em tempo real contra compartilhamento direto de celular, e-mail e links externos. O indicador está em modo observador acompanhando o progresso para garantir a comissão.
                    </p>
                  </div>
                </div>
                ...
                <div className="border border-slate-200 rounded-2xl overflow-hidden bg-slate-50 flex flex-col h-[280px]">
                  ...
                  <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
                    {(() => {
                      const leadMessages = chatMessages.filter(msg => msg.leadId === viewingLead.id);
                      if (leadMessages.length === 0) {
                        return <p className="text-center text-xs text-slate-400 py-8">Não há histórico de conversas para este lead.</p>;
                      }
                          ...
                          <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[75%] rounded-2xl px-3 py-2 text-xs shadow-sm ${
                              ...
                              <span className="font-bold text-[9px] block opacity-85 mb-0.5">
                                {isMe ? 'Você (Anunciante)' : `Comprador (${viewingLead.clientName})`}
                              </span>
                  ...
                  <form 
                    onSubmit={(e) => {
                      ...
                      onSendChatMessage(viewingLead.id, advertiser.id, advertiser.name, 'advertiser', advertiserChatText.trim());
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="font-display font-bold text-slate-900 text-lg">Agendamento de Visita Física</h2>
              <p className="text-xs text-slate-500 mt-1">Insira os dados acordados de data, horário e observações.</p>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-150 text-xs mb-4 space-y-1">
              ...
              <p className="text-[10px] text-slate-500 font-mono mt-0.5">{schedulingLead.clientPhone}</p>
              <p className="text-[10px] text-orange-600 font-bold block truncate uppercase mt-1">Interesse: {schedulingLead.productTitle}</p>
            </div>
            ...
            <form onSubmit={(e) => {
              ...
              <div className="text-xs">
                <label className="block text-[11px] font-semibold text-slate-700 mb-1">Observações do Cliente / Notas de Visita</label>
                <textarea 
                  rows={3}
