...
import { 
  Share2, MapPin, Wallet, Award, ArrowUpRight, DollarSign, Filter, Info, 
  CheckCircle, FileText, Smartphone, RefreshCw, Navigation, Camera, Check, Copy, AlertTriangle,
...
interface AffiliateDashboardProps {
  ...
  onAddSimulation: (sim: Omit<FinancingSimulation, 'id' | 'createdAt' | 'updatedAt' | 'status'>) => void;
...
export default function AffiliateDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'vitrine' | 'desempenho' | 'carteira' | 'financiamentos'>('vitrine');
  ...
  const [whatsAppNotificationData, setWhatsAppNotificationData] = useState<{ advertiserPhone: string; advertiserName: string; text: string } | null>(null);
  ...
  const [selectedSocialPlatform, setSelectedSocialPlatform] = useState<'instagram' | 'whatsapp' | 'facebook' | 'tiktok' | 'linkedin' | ''>('');
  ...
  const handleRegisterOnboard = (e: FormEvent) => {
    e.preventDefault();
    if (!scrolledTerms) {
      onAddNotification('Você precisa rolar os termos de parceria até o final para aceitar.', 'info');
      return;
    }
  ...
  const handleSimFormSubmit = (e: FormEvent) => {
    ...
    const formattedIncome = (parseFloat(simFormClientIncome) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    const formattedDownPayment = (parseFloat(simFormDownPayment) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
  ...
  const handleRequestWithdraw = (e: FormEvent) => {
    ...
    setTimeout(() => {
      ...
      onAddNotification(`Saque de R$ ${amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} enviado para sua chave PIX!`, 'success');
  ...
  const simulateSocialShare = (method: string, prod: Product) => {
    ...
    if (method === 'whatsapp') {
      const text = encodeURIComponent(`Olá! Veja este excelente item: ${prod.title}\nConfira todos os detalhes aqui: ${trackingUrl}`);
      window.open(`https://api.whatsapp.com/send?text=${text}`, '_blank');
      onAddNotification('Redirecionando para o WhatsApp...', 'success');
    } else if (method === 'instagram') {
      onAddNotification('Iniciando Kit Instagram: Baixe a arte e anexe o link nos Stories!', 'info');
    }
  };
  ...
  const downloadAllImages = (gallery: string[]) => {
    ...
    onAddNotification(`Abrindo ${gallery.length} imagem(ns) do produto para você salvar no seu dispositivo!`, 'success');
  };
  ...
  const handleGpsCheckIn = (lead: Lead) => {
    ...
    if (!navigator.geolocation) {
      setGpsError('Seu navegador não suporta geolocalização. Use a foto de fachada como alternativa.');
      setGpsLoading(false);
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setTimeout(() => {
          ...
          onAddNotification(`Check-in confirmado com sucesso! Distância de 42m do local.`, 'success');
      ...
      (error) => {
        ...
        setTimeout(() => {
          ...
          setGpsSuccess(true);
          onUpdateLeadStatus(lead.id, 'visita_confirmada');
          onAddNotification(`Check-in simulado com sucesso! (GPS de demonstração autorizado)`, 'success');
      ...
      <div className="max-w-md mx-auto my-8 bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 font-sans">
        ...
        <form onSubmit={handleRegisterOnboard} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">WhatsApp</label>
              <input 
                type="text" required
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Cidade de Atuação</label>
              <input 
                type="text" required
          ...
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
            <span className="block text-xs font-bold text-slate-800 mb-2">Configurar Chave PIX (Para Recebimento)</span>
                ...
                <button
                  ...
                  className={`px-2 py-1.5 rounded-lg text-xs font-medium border capitalize transition-all ${
                    onboardForm.pixType === type
                      ? 'bg-orange-600 border-orange-600 text-white shadow shadow-orange-100'
            ...
            <input 
              ...
              className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider">Contrato de Parceria Comercial Autônoma</label>
            ...
            >
              <p className="font-bold text-white mb-2 uppercase text-center border-b border-slate-800 pb-1">CONTRATO DE PARCERIA COMERCIAL AUTÔNOMA</p>
              <p className="mb-2"><strong className="text-orange-400">CLÁUSULA 1ª - DA AUTONOMIA REAL:</strong> O parceiro comercial (Indicador) atuará de forma totalmente independente e sem subordinação. O parceiro é livre para fixar seus próprios dias, horários e locais de atividade, não estando sujeito a carga horária de trabalho mínima ou obrigatoriedade de presença.</p>
              <p className="mb-2"><strong className="text-orange-400">CLÁUSULA 2ª - DA MULTIPLICIDADE:</strong> É garantido ao Indicador o direito de exercer atividades correlatas para outros anunciantes, imobiliárias, concessionárias ou plataformas concorrentes de forma simultânea.</p>
              <p className="mb-2"><strong className="text-orange-400">CLÁUSULA 3ª - DA EXCLUSÃO DE VÍNCULO:</strong> Este contrato não cria sob qualquer hipótese qualquer tipo de vínculo de emprego, relação de subordinação ou representação estatutária, conforme o Art. 442-B da CLT.</p>
              <p className="mb-2"><strong className="text-orange-400">CLÁUSULA 4ª - DA ATUAÇÃO DE IMÓVEIS (LEI 6.530/78):</strong> Fica estabelecido que na vertical de imóveis, a função do Indicador é exclusivamente de geração de leads/indicação qualificada. Qualquer ato restrito de corretagem imobiliária ou intermediação imobiliária comercial depende de registro ativo no Conselho Regional de Corretores de Imóveis (CRECI).</p>
          ...
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-start gap-3">
            <input 
              type="checkbox" required id="creci_checkbox"
              checked={creciWarningAccepted}
              onChange={e => setCreciWarningAccepted(e.target.checked)}
              className="mt-1 w-4 h-4 text-orange-600 border-slate-300 rounded focus:ring-orange-500 focus:outline-none"
            />
            <label htmlFor="creci_checkbox" className="text-[11px] text-slate-600 leading-tight">
              Entendo que a indicação de <strong>Imóveis</strong> se limita à indicação de leads interessados, e não substitui a intermediação profissional garantida por Corretores credenciados (CRECI).
            </label>
          </div>
  ...
  const filteredProducts = products.filter(prod => {
    ...
    if (cityFilter && !prod.location.city.toLowerCase().includes(cityFilter.toLowerCase())) return false;
    if (minCommission > 0) {
      const bestCommission = prod.allowPresencialTier && prod.commissionPresencialValue
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 font-sans">
      ...
      <div className="bg-gradient-to-r from-orange-950 via-slate-900 to-slate-950 rounded-3xl p-6 text-white mb-8 shadow-xl border border-orange-900/20 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-6 relative z-10">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-orange-600 rounded-2xl flex items-center justify-center font-bold text-2xl uppercase shadow-md border-2 border-orange-500 text-white">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                  indicator.league === 'ouro' ? 'bg-amber-400 text-amber-950' :
                  indicator.league === 'prata' ? 'bg-slate-300 text-slate-950' : 'bg-orange-400 text-orange-950'
                }`}>
                  Liga {indicator.league}
                </span>
              </div>
              <p className="text-xs text-orange-200 mt-0.5 font-mono flex items-center gap-2 flex-wrap">
                <span>{indicator.email} • Reputação: {indicator.score}/100</span>
              ...
              <p className="text-[10px] text-slate-300 mt-1 font-mono">Chave PIX: {indicator.pixKey} ({indicator.pixType.toUpperCase()})</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-slate-950/40 p-4 rounded-2xl border border-white/5 backdrop-blur-sm self-start md:self-auto">
            <div>
              <span className="block text-[10px] text-slate-400 uppercase font-bold">Saldo Disponível</span>
              <span className="text-2xl font-mono font-bold text-emerald-400">R$ {indicator.balanceAvailable.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="border-l border-slate-800 h-10 pl-4">
              <span className="block text-[10px] text-slate-400 uppercase font-bold">A Receber</span>
              <span className="text-sm font-mono font-semibold text-amber-400">R$ {indicator.balancePending.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 mb-6 font-display font-medium text-sm">
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'vitrine' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'desempenho' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-4 border-b-2 transition-all ${
            activeTab === 'carteira' ? 'border-orange-600 text-orange-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
          Carteira / PIX
        </button>
        ...
        >
          Simular Financiamentos ({simulations?.filter(s => s.indicatorId === indicator.id).length || 0})
        </button>
      </div>
      ...
      {scheduledVisits.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex gap-3">
            ...
            <div>
              <h4 className="font-bold text-sm text-amber-900">Visitas Presenciais Pendentes ({scheduledVisits.length})</h4>
              <p className="text-xs text-amber-700 mt-0.5">Você tem visitas agendadas. Lembre-se de realizar o check-in por GPS no local para ativar sua comissão!</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-5 lg:col-span-1">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <button 
                ...
                className="text-xs text-slate-400 hover:text-orange-600 font-medium"
              >
                Limpar
              </button>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Comissão Mínima (R$)</label>
              <select
                value={minCommission}
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
              <input 
                type="checkbox" id="only_presencial"
                checked={onlyPresencial}
                onChange={e => setOnlyPresencial(e.target.checked)}
                className="w-4 h-4 text-orange-600 rounded border-slate-300 focus:ring-orange-500"
              />
              <label htmlFor="only_presencial" className="text-xs text-slate-600 select-none font-semibold">
            ...
            {indicator.city && (
              <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                <input 
                  type="checkbox" id="filter_my_region"
                  checked={filterByMyRegion}
                  onChange={e => setFilterByMyRegion(e.target.checked)}
                  className="w-4 h-4 text-orange-600 rounded border-slate-300 focus:ring-orange-500"
                />
                <label htmlFor="filter_my_region" className="text-xs text-slate-600 select-none font-semibold flex items-center gap-1">
          ...
          <div className="lg:col-span-3 space-y-4">
            {(() => {
              ...
              const sortedRegionalHighlights = [...regionalHighlights].sort((a, b) => {
                const aCityMatch = userCityClean && a.location.city.toLowerCase().includes(userCityClean) ? 1 : 0;
                const bCityMatch = userCityClean && b.location.city.toLowerCase().includes(userCityClean) ? 1 : 0;
              ...
              return (
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-3xl p-5 border border-orange-200/60 shadow-xs mb-6">
                  <div className="flex items-center justify-between mb-3.5">
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        <span className="absolute inline-flex h-2.5 w-2.5 rounded-full bg-orange-400 opacity-75 animate-ping"></span>
                        <MapPin className="w-5 h-5 text-orange-600 relative" />
                      </div>
                      <div>
                        <h4 className="font-display font-bold text-sm text-slate-900 flex items-center gap-1.5">
                          Novidades na Sua Região <span className="text-orange-600 font-extrabold font-mono text-xs bg-orange-100 px-2.5 py-0.5 rounded-full uppercase">{indicator.city} - {indicator.state}</span>
                        </h4>
                        <p className="text-[11px] text-slate-500 mt-0.5">Identificamos {regionalHighlights.length} produtos em toda a sua localidade e categorias associadas.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {sortedRegionalHighlights.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {sortedRegionalHighlights.map(prod => {
                        const bestComm = prod.allowPresencialTier && prod.commissionPresencialValue
                          ? prod.commissionPresencialValue
                          : (prod.commissionDigitalValue || 0);

                        return (
                          <div key={`highlight-${prod.id}`} className="bg-white rounded-2xl p-3 border border-orange-100 hover:border-orange-200/80 shadow-xs transition-all flex flex-col justify-between">
                            <div>
                              <div className="relative aspect-video rounded-xl overflow-hidden bg-slate-100 mb-2">
                                ...
                                />
                                <div className="absolute top-1.5 left-1.5 bg-orange-600 text-white px-2 py-0.5 rounded-md text-[8px] font-bold font-mono uppercase">
                                  {prod.category}
                                </div>
                              ...
                              <h5 className="font-bold text-xs text-slate-900 line-clamp-1 leading-tight">{prod.title}</h5>
                              <p className="text-[10px] text-slate-400 mt-0.5 line-clamp-1">{prod.advertiserName}</p>
                            </div>

                            <div className="mt-2.5 pt-2 border-t border-slate-50 flex items-center justify-between">
                              <div>
                                <span className="text-[8px] text-slate-400 uppercase block font-bold leading-none">Comissão</span>
                                <span className="text-xs font-mono font-bold text-emerald-600 leading-none">R$ {bestComm.toLocaleString('pt-BR')}</span>
                              </div>
                              <div className="flex gap-1">
                                ...
                                <button
                                  ...
                                  className="bg-orange-600 hover:bg-orange-500 text-white font-bold text-[10px] py-1.5 px-2.5 rounded-lg transition-all flex items-center gap-1 shadow-sm"
                                >
                    ...
                    <div className="bg-white/80 rounded-2xl p-4 border border-orange-100 text-center">
                      <p className="text-xs text-slate-600 leading-relaxed font-sans">
                        💡 <strong className="font-semibold text-orange-950">Sabedoria de Profissional:</strong> Não há ofertas ativas em {indicator.city} atualmente, mas você pode indicar e simular produtos de qualquer região do Brasil para ganhar comissões agora mesmo!
                      </p>
                    </div>
                  )}
                </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center bg-white p-12 border border-slate-150 rounded-3xl">
                ...
                <p className="font-semibold text-slate-800 text-base">Nenhum produto ativo encontrado</p>
                <p className="text-xs text-slate-500 mt-1">Tente afrouxar os filtros de busca para visualizar mais opções.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredProducts.map(prod => {
                  ...
                  return (
                    <div key={prod.id} className="bg-white rounded-3xl overflow-hidden border border-slate-100 hover:border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col">
                      <div className="relative h-44 overflow-hidden bg-slate-900">
                        ...
                        {prod.status === 'reservado' && (
                          <div className="absolute inset-0 bg-slate-950/60 flex items-center justify-center">
                            <span className="bg-amber-600 text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest font-display">RESERVADO</span>
                          </div>
                        )}
                        <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-sm rounded-lg py-1 px-2.5 text-xs font-semibold text-slate-200 border border-white/5 flex items-center gap-1">
                          ...
                          <span>{prod.location.city} - {prod.location.state}</span>
                        </div>
                      </div>

                      <div className="p-4 flex-1 flex flex-col justify-between">
                        <div>
                          <span className="text-[10px] text-orange-600 font-bold tracking-wider uppercase font-mono">{prod.advertiserName}</span>
                          <h3 className="font-display font-bold text-slate-900 text-base leading-tight mt-0.5 line-clamp-1">{prod.title}</h3>
                          <p className="text-xs text-slate-500 mt-1 line-clamp-2">{prod.description}</p>
                          
                          <div className="mt-3 font-mono">
                            <span className="text-[10px] text-slate-400 uppercase font-bold block">Valor de Venda</span>
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-100">
                          <div>
                            <span className="text-[9px] text-slate-500 font-bold block uppercase tracking-wider">Digital (Indicação)</span>
                            <span className="text-xs font-mono font-bold text-orange-700">
                              R$ {digitalComm.toLocaleString('pt-BR')} <span className="text-[9px] font-normal text-slate-500">({prod.commissionDigitalPct}%)</span>
                            </span>
                          </div>
                          <div className="border-l border-slate-200 pl-2">
                            <span className="text-[9px] text-slate-500 font-bold block uppercase tracking-wider">Presencial (Acompanha)</span>
                            <span className={`text-xs font-mono font-bold ${prod.allowPresencialTier ? 'text-emerald-700' : 'text-slate-400'}`}>
                              {prod.allowPresencialTier ? (
                                <>R$ {presencialComm.toLocaleString('pt-BR')} <span className="text-[9px] font-normal text-slate-500">({prod.commissionPresencialPct}%)</span></>
                              ) : (
                                'Não permitido'
                              )}
                            </span>
                        ...
                        <div className="mt-4 flex gap-2">
                          ...
                          <button
                            onClick={() => setSharingProduct(prod)}
                            className="flex-1 bg-orange-600 hover:bg-orange-500 text-white font-semibold text-xs py-2.5 px-3 rounded-xl transition-all shadow flex items-center justify-center gap-1.5 shadow-orange-100"
                          >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-150 p-6 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <span className="text-[10px] text-orange-600 uppercase font-bold tracking-wider block">Sistema de Ligas</span>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Comissões 15% maiores ativas por reputação excelente.</p>
                </div>
              </div>
            </div>

            <div className="border-t md:border-t-0 md:border-l md:border-r border-slate-200 py-4 md:py-0 md:px-6 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Próximo Nível</span>
              <div className="flex justify-between text-xs font-semibold text-slate-700">
                <span>Liga Suprema (Breve)</span>
                <span>85% Concluído</span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div className="bg-orange-600 h-full w-[85%] rounded-full"></div>
              </div>
              <p className="text-[10px] text-slate-400">Indique mais 2 vendas qualificadas para desbloquear.</p>
            </div>

            <div className="space-y-1 flex flex-col justify-center">
              <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block">Atividades Acumuladas</span>
              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                  <span className="text-base font-bold font-mono text-slate-950">{indicator.clicks}</span>
                  <span className="text-[9px] text-slate-500 block uppercase">Cliques</span>
                </div>
                <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                  <span className="text-base font-bold font-mono text-slate-950">{activeLeads.length}</span>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm overflow-hidden">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
              <div>
                <h3 className="font-display font-bold text-slate-900 text-lg">Seus Leads e Indicações</h3>
                <p className="text-xs text-slate-500">Acompanhe a atribuição de contatos, visitas e fechamentos em tempo real.</p>
              </div>
            </div>
            ...
            {activeLeads.length === 0 ? (
              <div className="text-center p-12 text-slate-400">
                ...
                <p className="text-xs mt-1">Compartilhe links da vitrine para ver os leads surgindo aqui!</p>
              </div>
            ) : (
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 font-sans">
                    {activeLeads.map(lead => {
                      ...
                      return (
                        <>
                          <tr key={lead.id} className={`hover:bg-slate-50/50 transition-all ${isChatOpen ? 'bg-orange-50/20' : ''}`}>
                            <td className="py-4 px-4 font-medium text-slate-900">
                              <div>{lead.clientName}</div>
                              <div className="text-[10px] text-slate-400 mt-0.5">{lead.clientPhone} • {lead.clientEmail}</div>
                            </td>
                            <td className="py-4 px-4 text-slate-600">
                              <span className="font-semibold line-clamp-1">{lead.productTitle}</span>
                              <span className="text-[10px] text-slate-400 capitalize bg-slate-100 rounded px-1.5 py-0.5 mt-1 inline-block">{lead.productCategory}</span>
                            </td>
                            <td className="py-4 px-4 font-mono font-semibold uppercase text-[10px]">
                              {lead.commissionType === 'presencial' ? (
                                <span className="text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full">Presencial</span>
                              ) : (
                                <span className="text-orange-600 bg-orange-50 border border-orange-100 px-2 py-0.5 rounded-full">Digital</span>
                              )}
                            </td>
                            ...
                            <td className="py-4 px-4">
                              <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                                lead.status === 'venda_concluida' ? 'bg-emerald-100 text-emerald-800' :
                                lead.status === 'visita_confirmada' ? 'bg-cyan-100 text-cyan-800' :
                                lead.status === 'visita_agendada' ? 'bg-amber-100 text-amber-800' :
                                lead.status === 'proposta' ? 'bg-orange-100 text-orange-800' : 'bg-slate-100 text-slate-700'
                              }`}>
                              ...
                              <div className="flex items-center justify-end gap-1.5 flex-wrap">
                                {lead.status === 'visita_agendada' && (
                                  lead.checkInRequested ? (
                                    ...
                                    <button
                                      onClick={() => {
                                        onUpdateLeadStatus(lead.id, 'visita_agendada', { checkInRequested: true });
                                        onAddNotification('Você sinalizou que chegou à loja com o cliente! Uma notificação foi enviada ao lojista para confirmar.', 'success');
                                      }}
                                      className="bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-bold py-1 px-2.5 rounded-lg transition-all text-[9px] flex items-center gap-1 shadow-sm active:scale-95"
                                    >
                                ...
                                >
                                  ...
                                  {leadMsgs.length > 0 && (
                                    <span className="bg-orange-100 text-orange-800 text-[8px] font-bold rounded-full px-1.5 py-0.2">
                                      {leadMsgs.length}
                                    </span>
                                  )}
                                </button>
                          ...
                          {isChatOpen && (
                            <tr key={`${lead.id}-chat-expanded`} className="bg-slate-50/50">
                              <td colSpan={6} className="p-4 bg-slate-50/60 border-t border-b border-slate-100">
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden max-w-4xl mx-auto">
                                  ...
                                  <div className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div className="text-left">
                                        <h4 className="font-bold text-xs text-slate-100">Console de Monitoramento • {lead.clientName}</h4>
                                        <p className="text-[9px] text-slate-400">Atendimento garantido com indicação de {indicator.name}</p>
                                      </div>
                                    </div>
                                  ...
                                  {/* Info banner */}
                                  <div className="bg-orange-50 border-b border-orange-100 px-4 py-2 flex items-start gap-2 text-[10px] text-orange-950 text-left">
                                    <Info className="w-3.5 h-3.5 text-orange-600 flex-shrink-0 mt-0.5" />
                                    <span>
                                      Você está visualizando a conversa entre o lojista e o comprador em tempo real. Se o lojista concluir a venda, a sua comissão de <strong>R$ {lead.commissionValue.toLocaleString('pt-BR')}</strong> será liberada de imediato.
                                    </span>
                                  </div>
                                  ...
                                  <div className="p-4 space-y-3 max-h-[250px] overflow-y-auto bg-slate-50/40 text-left flex flex-col">
                                    {leadMsgs.length === 0 ? (
                                      <p className="text-center text-xs text-slate-400 py-6">Ainda não há mensagens registradas neste atendimento.</p>
                                    ) : (
                                      leadMsgs.map(msg => {
                                          ...
                                          <div key={msg.id} className={`flex ${isClient ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[75%] rounded-2xl px-3 py-2 text-xs shadow-sm ${
                                              isClient 
                                                ...
                                                : 'bg-orange-50 border border-orange-100 text-orange-950 rounded-bl-none'
                                            }`}>
                                              <div className="flex items-center justify-between gap-4 mb-1">
                                                <span className="font-bold text-[9px] uppercase tracking-wider text-slate-400">
                                                  {isClient ? `Comprador (${lead.clientName})` : `Anunciante / Loja`}
                                                </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6 lg:col-span-1">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center font-mono">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Disponível para Resgate</span>
              <span className="text-3xl font-bold text-emerald-600">
                R$ {indicator.balanceAvailable.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </span>
              <span className="text-[10px] text-slate-400 block mt-1">Chave cadastrada: {indicator.pixKey}</span>
            </div>

            <form onSubmit={handleRequestWithdraw} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Valor de Saque (R$)</label>
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-mono text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                />
              </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm lg:col-span-2 space-y-4">
            <h3 className="font-display font-bold text-slate-900 text-base">Histórico de Movimentação Financeira</h3>
            <p className="text-xs text-slate-500">Transações de comissões, faturamentos, adiantamentos e pagamentos recebidos.</p>

            <div className="space-y-3">
              {activeLeads.filter(l => l.commissionPaid).map(lead => (
                <div key={lead.id} className="flex justify-between items-center border border-slate-100 rounded-2xl p-4 bg-slate-50 hover:bg-slate-100/50 transition-all">
                  <div className="flex gap-3">
                    <div className="bg-emerald-100 text-emerald-800 w-10 h-10 rounded-xl flex items-center justify-center font-bold">PIX</div>
                    <div>
                      <h4 className="font-bold text-sm text-slate-900">Comissão de Venda Creditada</h4>
                      <p className="text-xs text-slate-500">{lead.productTitle} • Cliente: {lead.clientName}</p>
                    </div>
                  </div>
                  <span className="font-mono font-bold text-emerald-600 text-sm">
                    + R$ {lead.commissionValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              ))}

              {activeLeads.filter(l => !l.commissionPaid && l.status === 'venda_concluida').map(lead => (
                <div key={lead.id} className="flex justify-between items-center border border-slate-150 rounded-2xl p-4 bg-white hover:bg-slate-50 transition-all">
                  <div className="flex gap-3">
                    <div className="bg-amber-100 text-amber-800 w-10 h-10 rounded-xl flex items-center justify-center font-bold font-mono text-[10px]">PND</div>
                    <div>
                      <h4 className="font-bold text-sm text-slate-900">Comissão de Venda Pendente de Repasse</h4>
                      <p className="text-xs text-slate-500">{lead.productTitle} • Aguardando prazo de repasse da loja</p>
                    </div>
                  </div>
      ...
      {activeTab === 'financiamentos' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-gradient-to-r from-orange-500 to-amber-600 rounded-3xl p-6 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2 max-w-xl">
              <div className="flex items-center gap-2">
                <Landmark className="w-6 h-6 text-white" />
                <h3 className="font-display font-bold text-lg md:text-xl">Intermediação de Crédito Veicular</h3>
              </div>
              <p className="text-xs text-orange-50 leading-relaxed">
                Intermedeie o financiamento do comprador diretamente com a concessionária ou bancos parceiros. 
                Cadastre os dados de crédito do comprador para que a loja faça a simulação automática multi-bancos. 
                Quando aprovado, você visualiza as parcelas e termos para fechar o negócio com total transparência!
              </p>
            </div>
            <button
              onClick={() => {
                const vehicleProducts = products.filter(p => ['carro', 'moto', 'barco', 'jetski'].includes(p.category));
              ...
              className="bg-white text-orange-950 hover:bg-orange-50 font-bold text-xs py-3 px-5 rounded-2xl transition-all shadow-md flex items-center gap-2 self-start md:self-center shrink-0 active:scale-95"
            >
          ...
          <div className="space-y-6">
            <h3 className="font-display font-bold text-slate-950 text-base flex items-center gap-2">
              ...
              Simulações e Status de Crédito ({simulations?.filter(s => s.indicatorId === indicator.id).length || 0})
            </h3>

            <div className="grid grid-cols-1 gap-6">
              {(simulations || []).filter(s => s.indicatorId === indicator.id).map(sim => {
                const product = products.find(p => p.id === sim.productId);
                return (
                  <div key={sim.id} className="bg-white rounded-3xl border border-slate-100 shadow-xs hover:shadow-md transition-all overflow-hidden flex flex-col">
                    ...
                    <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-sm text-slate-900 leading-tight">{sim.productTitle}</h4>
                          <span className="text-[11px] text-slate-500 font-mono">
                            Preço do Veículo: <strong className="font-semibold text-slate-700">R$ {sim.productPrice.toLocaleString('pt-BR')}</strong>
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100 text-xs">
                        ...
                        <div className="grid grid-cols-2 gap-3 leading-relaxed">
                          ...
                          <div>
                            ...
                            <span className="font-semibold text-slate-950 font-mono">{sim.clientBirthDate}</span>
                          </div>
                          <div>
                            <span className="text-slate-400 block font-medium">Renda Mensal:</span>
                            <span className="font-semibold text-emerald-700 font-mono">R$ {sim.clientIncome.toLocaleString('pt-BR')}</span>
                          </div>
                          <div>
                            <span className="text-slate-400 block font-medium">Entrada Proposta:</span>
                            <span className="font-semibold text-slate-900 font-mono">R$ {sim.downPayment.toLocaleString('pt-BR')}</span>
                          </div>
                        </div>
                        <div className="pt-2 border-t border-slate-200">
                          <span className="text-slate-400 block">Prazo Desejado:</span>
                          <span className="font-semibold text-slate-900">{sim.desiredInstallments} meses</span>
                        </div>
                      </div>
                      ...
                      <div className="lg:col-span-2 flex flex-col justify-between h-full space-y-4">
                        {sim.status === 'pendente' && (
                          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-amber-50/20 border border-dashed border-amber-200 rounded-2xl">
                            ...
                            <h5 className="font-bold text-sm text-amber-900">Enviado para a Mesa de Crédito</h5>
                            <p className="text-xs text-slate-500 max-w-sm mt-1">
                              A loja parceira já recebeu este pedido e está enviando os dados do comprador para as mesas de análise dos bancos. Fique atento para atualizações!
                            </p>
                          </div>
                        )}
                        ...
                        {sim.status === 'rejeitado' && (
                          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-red-50/20 border border-dashed border-red-200 rounded-2xl">
                            ...
                            <h5 className="font-bold text-sm text-red-900">Análise de Crédito Recusada</h5>
                            <p className="text-xs text-slate-500 max-w-sm mt-1">
                              Infelizmente, as instituições financeiras consultadas recusaram a liberação de crédito para este perfil de renda/score. Tente re-ajustar o valor da entrada.
                            </p>
                          </div>
                        )}

                        {/* If banks under review or approved: show multi-bank response options */}
                        {(sim.status === 'analise_bancos' || sim.status === 'aprovado' || sim.status === 'concluido') && (
                          <div className="space-y-3">
                                  ...
                                  >
                                    <div>
                                      <div className="flex justify-between items-center mb-1">
                                        ...
                                        {bank.approvedStatus === 'aprovado' && (
                                          <span className="bg-emerald-100 text-emerald-800 text-[8px] font-extrabold px-1.5 py-0.5 rounded font-mono">OK</span>
                                        )}
                                      </div>
                                      <p className="font-mono text-[10px] text-slate-500 mt-1">
                                        Financia: <strong className="font-bold text-slate-700">R$ {bank.approvedAmount.toLocaleString('pt-BR')}</strong>
                                      </p>
                                      <p className="font-mono text-[10px] text-slate-500">
                                        Taxa: <strong className="font-bold text-orange-600">{bank.interestRate}% a.m.</strong>
                                      </p>
                                    </div>

                                    <div className="pt-2 border-t border-slate-100/60 mt-2">
                                      <span className="text-[10px] text-slate-400 block">Parcelas:</span>
                                      <span className="text-sm font-bold font-mono text-slate-900">
                                        {bank.installmentsCount}x R$ {bank.installmentValue.toLocaleString('pt-BR')}
                                      </span>
                                    </div>
                                  </div>
                                ))
                          ...
                          <div className="bg-gradient-to-tr from-slate-900 to-slate-950 text-white rounded-2xl p-4 border border-slate-800 space-y-3">
                            <div className="flex justify-between items-center border-b border-white/10 pb-2">
                              <span className="text-[9px] font-bold uppercase tracking-widest text-emerald-400 flex items-center gap-1 font-mono">
                                <CheckCircle className="w-3.5 h-3.5 text-emerald-400" /> Contrato de Crédito Ativo
                              </span>
                              <span className="font-bold text-xs text-slate-300 font-mono">{sim.approvedContract.bankName}</span>
                            </div>

                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                              <div>
                                <span className="text-slate-400 block text-[9px] uppercase font-semibold">Valor Financiado</span>
                                <span className="font-bold text-white font-mono text-sm">R$ {sim.approvedContract.approvedAmount.toLocaleString('pt-BR')}</span>
                              </div>
                              <div>
                                <span className="text-slate-400 block text-[9px] uppercase font-semibold">Entrada Requerida</span>
                                <span className="font-bold text-white font-mono text-sm">R$ {sim.approvedContract.downPaymentRequired.toLocaleString('pt-BR')}</span>
                              </div>
                              <div>
                                <span className="text-slate-400 block text-[9px] uppercase font-semibold">Parcelas Aprovadas</span>
                                <span className="font-bold text-emerald-400 font-mono text-sm">{sim.approvedContract.installmentsCount}x R$ {sim.approvedContract.installmentValue.toLocaleString('pt-BR')}</span>
                              </div>
                              <div>
                                <span className="text-slate-400 block text-[9px] uppercase font-semibold">Taxa de Juros</span>
                                <span className="font-bold text-orange-400 font-mono text-sm">{sim.approvedContract.interestRate}% a.m.</span>
                              </div>
                            </div>

                            {sim.approvedContract.additionalNotes && (
                              <p className="text-[10px] text-slate-300 bg-white/5 p-2 rounded-lg border border-white/5">
                                💡 <strong className="font-semibold text-slate-100">Notas da Loja:</strong> {sim.approvedContract.additionalNotes}
                              </p>
                            )}

                            <div className="flex justify-between items-center pt-1 text-[10px] text-slate-400">
                              <span>Sua comissão está garantida na assinatura eletrônica deste contrato.</span>
                              <span className="text-emerald-400 font-bold font-mono">CRÉDITO SEGURO</span>
                            </div>
                          </div>
                        )}
                      </div>
              ...
              {(simulations || []).filter(s => s.indicatorId === indicator.id).length === 0 && (
                <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 p-8">
                  ...
                  <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 leading-relaxed">
                    Você ainda não cadastrou nenhuma simulação de financiamento para intermediar com compradores. Selecione um veículo de interesse e clique em "Nova Simulação de Crédito".
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 relative font-sans shadow-2xl max-h-[95vh] overflow-y-auto">
            ...
            <div className="text-center mb-6 border-b border-slate-100 pb-4">
              <span className="text-[10px] bg-orange-50 border border-orange-100 text-orange-700 px-3 py-1 rounded-full font-bold uppercase tracking-wider">
                Intermediação Direta de Financiamento
              </span>
              <h2 className="font-display font-bold text-slate-900 text-xl mt-2">Simular Financiamento do Comprador</h2>
              <p className="text-xs text-slate-500 mt-1">Insira os dados cadastrais do comprador do veículo para aprovação de crédito na loja parceira.</p>
            </div>
            ...
            <form onSubmit={handleSimFormSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-medium"
                >
              ...
              <div className="border-t border-slate-100 pt-4 space-y-4">
                <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Dados do Comprador (Ficha de Crédito)</span>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Nome Completo do Cliente</label>
                    <input 
                      type="text" required
                      value={simFormClientName}
                      onChange={e => setSimFormClientName(e.target.value)}
                      placeholder="ex: Roberto Alencar"
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">CPF do Comprador</label>
                    <input 
                      type="text" required
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">WhatsApp de Contato</label>
                    <input 
                      type="text" required
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Data de Nascimento</label>
                    <input 
                      type="date" required
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Renda Mensal Comprovada (R$)</label>
                    <input 
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Valor de Entrada Ofertado (R$)</label>
                    <input 
                      type="number" required min="0" step="100"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">Prazo Desejado (Nº Parcelas)</label>
                    <select 
                      value={simFormDesiredInstallments}
                      onChange={e => setSimFormDesiredInstallments(parseInt(e.target.value))}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-medium"
                    >
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-start gap-2.5 mt-2">
                ...
                <p className="text-[10px] text-slate-500 leading-snug">
                  Ao enviar esta simulação de crédito, você autoriza a plataforma a enviar estes dados cadastrais de crédito diretamente ao sistema da loja do veículo. A loja irá realizar a consulta multi-bancos. Seus dados cadastrais estão protegidos pela LGPD.
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-orange-600 hover:bg-orange-500 text-white py-3 rounded-xl font-semibold text-xs transition-all shadow shadow-orange-100 flex items-center justify-center gap-1"
                >
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-6 border-b border-slate-100 pb-4">
              ...
              <h2 className="font-display font-bold text-slate-900 text-xl mt-2">Pronto para Compartilhar</h2>
              <p className="text-xs text-slate-500 mt-1">Sempre divulgue usando seu link pessoal de atribuição.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <span className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1.5">Seu Link de Atribuição Único</span>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="bg-orange-600 hover:bg-orange-500 text-white p-2.5 rounded-xl transition-all shadow shadow-orange-100"
                      title="Copiar Link"
                    >
                  ...
                  <p className="text-[10px] text-slate-400 mt-2 leading-snug">
                    ✓ Visitantes que clicarem neste link terão um cookie de atribuição de 60 dias gravado em seus navegadores.
                  </p>
                </div>

                {/* SOCIAL PLATFORM SELECTOR */}
                <div className="space-y-2">
                  <span className="block text-xs font-bold text-slate-700 uppercase tracking-wide">1. Escolha a Rede Social</span>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-center transition-all flex flex-col items-center gap-1 ${
                          selectedSocialPlatform === platform.id
                            ? 'border-slate-900 bg-slate-50 ring-2 ring-orange-500 ring-offset-1'
                            : 'border-slate-200 bg-white hover:bg-slate-50'
                        }`}
                      >
                        <span className="text-base">{platform.icon}</span>
                        <span className="text-[9px] font-bold text-slate-700">{platform.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {selectedSocialPlatform && (
                  <div className="space-y-2 animate-fade-in">
                    <span className="block text-xs font-bold text-slate-700 uppercase tracking-wide">2. Formato do Anúncio</span>
                    <div className="flex gap-2">
                      {selectedSocialPlatform === 'instagram' && (
                        <>
                          {['stories', 'feed', 'reels'].map(p => (
                            ...
                            >
                              {p === 'stories' ? 'Stories 📱' : p === 'feed' ? 'Feed 🖼️' : 'Reels 🎬'}
                            </button>
                          ))}
                        </>
                      )}
                      ...
                      {(selectedSocialPlatform === 'facebook' || selectedSocialPlatform === 'linkedin' || selectedSocialPlatform === 'tiktok') && (
                  ...
                  return (
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3.5 text-xs text-slate-800 animate-slide-in">
                      <div className="flex items-center gap-1.5 border-b border-slate-200 pb-1.5 mb-1">
                        ...
                        <span className="font-bold text-[10px] uppercase tracking-wider text-slate-500">Passos de Publicação</span>
                      </div>

                      {/* Step 1: Download Media */}
                      <div className="flex items-start gap-3">
                        <div className="bg-orange-100 text-orange-800 rounded-full w-5 h-5 flex items-center justify-center font-bold text-[10px] flex-shrink-0 mt-0.5">1</div>
                        <div className="flex-1">
                          <span className="block font-bold text-slate-800">Baixar Imagens do Produto</span>
                          <p className="text-[10px] text-slate-500 mb-2">Para criar um post visual marcante com a galeria de fotos.</p>
                          <button
                            type="button"
                          ...
                          >
                            <Camera className="w-3 h-3 text-slate-500" /> Baixar todas as {sharingProduct.gallery.length} fotos
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-3">
                        <div className="bg-orange-100 text-orange-800 rounded-full w-5 h-5 flex items-center justify-center font-bold text-[10px] flex-shrink-0 mt-0.5">2</div>
                        <div className="flex-1">
                          <span className="block font-bold text-slate-800">Copiar Legenda Adaptada</span>
                          <p className="text-[10px] text-slate-500">Texto formatado profissionalmente para obter mais cliques.</p>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              onAddNotification('Legenda comercial copiada com sucesso!', 'success');
                            }}
                      ...
                      <div className="flex items-start gap-3">
                        <div className="bg-orange-100 text-orange-800 rounded-full w-5 h-5 flex items-center justify-center font-bold text-[10px] flex-shrink-0 mt-0.5">3</div>
                        <div className="flex-1">
                          <span className="block font-bold text-slate-800">Postar na Rede Social</span>
                          <p className="text-[10px] text-slate-500 mb-2">Clique para abrir o aplicativo e criar sua publicação.</p>
                          <a
                            href={platformUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            onClick={() => onAddNotification(`Redirecionando para o ${selectedSocialPlatform.toUpperCase()}...`, 'success')}
                          ...
                          >
                            Abrir {selectedSocialPlatform} {selectedPlacement ? `(${selectedPlacement})` : ''} 🚀
                          </a>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 flex flex-col justify-between">
                <div>
                  <span className="block text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-2 text-center">Prévia do Post Personalizado (Kit)</span>
                  
                  {/* Visual card mimicking a premium Instagram Story post */}
                  <div id="promotion-card-preview" className="relative aspect-[3/4] bg-slate-950 rounded-xl overflow-hidden border border-slate-800 p-4 flex flex-col justify-between text-white shadow-xl">
                    ...
                    <div className="relative z-10 flex justify-between items-start">
                      <div className="bg-orange-600 text-white text-[9px] font-extrabold uppercase px-2.5 py-1 rounded-full font-mono tracking-widest shadow-sm">
                        Oportunidade
                      </div>
                    ...
                    <div className="relative z-10 space-y-1 text-left">
                      <p className="text-[10px] font-mono uppercase tracking-wider text-orange-400 font-bold">{sharingProduct.advertiserName}</p>
                      <h4 className="font-display font-bold text-sm tracking-tight line-clamp-2">{sharingProduct.title}</h4>
                ...
                <button
                  onClick={() => {
                    ...
                    setTimeout(() => {
                      ...
                      onAddNotification('Arte do Kit de Divulgação baixada com sucesso!', 'success');
                    }, 1200);
                  }}
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="font-display font-bold text-slate-900 text-lg">Check-In Presencial por GPS</h2>
              <p className="text-xs text-slate-500 mt-1">Confirme seu acompanhamento presencial do cliente.</p>
            </div>

            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2 mb-4 text-xs text-slate-700 font-sans">
              <div>
                <span className="font-bold block uppercase text-[9px] text-slate-400">Cliente Indicado</span>
                <p className="text-sm font-semibold text-slate-900">{checkingInLead.clientName}</p>
              </div>
              <div>
                <span className="font-bold block uppercase text-[9px] text-slate-400">Produto de Interesse</span>
                <p className="text-sm font-semibold text-slate-900">{checkingInLead.productTitle}</p>
              </div>
              <div>
                <span className="font-bold block uppercase text-[9px] text-slate-400">Comissão Adicional</span>
                <p className="text-sm font-bold text-emerald-600">R$ {checkingInLead.commissionValue.toLocaleString('pt-BR')} (Presencial)</p>
              </div>
            </div>
            ...
            {checkinMethod === 'gps' ? (
              <div className="space-y-4">
                <p className="text-xs text-slate-500 leading-relaxed">
                  O sistema usará o GPS do seu dispositivo para confirmar que você está no local físico do anúncio no horário agendado (janela de geolocalização do parceiro).
                </p>
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl font-semibold text-xs flex items-center justify-center gap-2 transition-all shadow ${
                    gpsLoading 
                      ...
                      : 'bg-orange-600 hover:bg-orange-500 text-white shadow shadow-orange-100'
                  }`}
                >
                  {gpsLoading ? (
                    <>
                ...
                {gpsError && <p className="text-[10px] text-red-500 text-center font-semibold">{gpsError}</p>}
              </div>
            ) : (
              <div className="space-y-4">
                <p className="text-xs text-slate-500 leading-relaxed">
                  Se o GPS estiver instável ou com imprecisões, tire uma foto da fachada da loja/imóvel com o cliente para auditoria posterior da plataforma.
                </p>

                <div className="border-2 border-dashed border-slate-200 hover:border-orange-500 rounded-2xl p-6 text-center transition-colors cursor-pointer bg-slate-50/50">
                  <input 
                    type="file" id="photo_checkin_upload" accept="image/*"
                    onChange={() => {
                      setMockPhotoUploaded(true);
                      onAddNotification('Foto da fachada anexada para auditoria de check-in!', 'info');
                    }}
                    className="hidden"
                  />
                  <label htmlFor="photo_checkin_upload" className="cursor-pointer space-y-1.5 block">
                    <Camera className="w-8 h-8 text-slate-400 mx-auto" />
                    <span className="block text-xs font-bold text-orange-600">Fotografar / Enviar Foto</span>
                ...
                >
                  {gpsLoading ? 'Analisando imagem...' : gpsSuccess ? 'Auditoria Concluída!' : 'Enviar Foto e Confirmar'}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 relative font-sans shadow-2xl">
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="font-display font-bold text-slate-900 text-lg">Notificar Anunciante via WhatsApp</h2>
              <p className="text-xs text-slate-500 mt-1">
                Uma mensagem contendo todos os dados do financiamento e o link de acesso foi gerada para você enviar diretamente via WhatsApp.
              </p>
            </div>
