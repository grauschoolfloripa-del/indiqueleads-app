...
function SponsorEmpty({
  ...
  if (variant === "strip") {
    return (
      <a
        href={href}
        className="group flex items-center justify-between gap-3 rounded-xl border border-dashed border-green-300 bg-green-50/60 px-3 py-2 text-[11px] font-semibold text-green-800 hover:bg-green-50 transition-colors"
      >
        <span className="flex items-center gap-2 min-w-0">
          <Sparkles className="w-3.5 h-3.5 shrink-0" />
          <span className="truncate">Espaço disponível para patrocinadores</span>
        </span>
        <span className="shrink-0 text-green-700 group-hover:underline">Anuncie aqui →</span>
      </a>
    );
  }
  ...
  if (variant === "card") {
    return (
      <a
        href={href}
        className="group block rounded-2xl border border-dashed border-green-300 bg-gradient-to-br from-green-50 to-white p-4 hover:shadow-md transition-all"
      >
        ...
        <span className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-green-700 group-hover:underline">
  ...
  return (
    <a
      href={href}
      className="group block overflow-hidden rounded-3xl border border-dashed border-green-300 bg-gradient-to-br from-green-50 via-white to-green-50/50 hover:shadow-lg transition-all"
    >
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-5 sm:p-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-green-600 text-white grid place-items-center shadow-md shrink-0">
        ...
        <div className="sm:ml-auto flex flex-col sm:items-end gap-1">
          ...
          <span className="inline-flex items-center gap-1 text-xs font-bold text-green-700 group-hover:underline">
...
function SponsorCardItem({ sponsor }: { sponsor: Sponsor }) {
  ...
  const className =
    "block rounded-2xl border border-slate-200 bg-white p-3 hover:border-green-300 hover:shadow-md transition-all";
        ...
        <div className="flex flex-wrap gap-2">
          {sponsors!.map((s) => (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-700 hover:border-green-300 hover:text-green-700"
            >
