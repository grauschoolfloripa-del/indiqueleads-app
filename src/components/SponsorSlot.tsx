...
function SponsorEmpty({
  ...
  const subject = encodeURIComponent("Quero patrocinar a IndiqueLeads");
  ...
  if (variant === "strip") {
    return (
      <a
        href={href}
        className="group flex items-center justify-between gap-3 rounded-xl border border-dashed border-blue-300 bg-blue-50/60 px-3 py-2 text-[11px] font-semibold text-blue-900 hover:bg-blue-50 transition-colors"
      >
        <span className="flex items-center gap-2 min-w-0">
          <Sparkles className="w-3.5 h-3.5 shrink-0" />
          <span className="truncate">Espaço disponível para patrocinadores</span>
        </span>
        <span className="shrink-0 text-blue-800 group-hover:underline">Anuncie aqui →</span>
      </a>
    );
  }
  ...
  if (variant === "card") {
    return (
      <a
        href={href}
        className="group block rounded-2xl border border-dashed border-blue-300 bg-gradient-to-br from-blue-50 to-white p-4 hover:shadow-md transition-all"
      >
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-blue-800">
        ...
        <span className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-blue-800 group-hover:underline">
  ...
  return (
    <a
      href={href}
      className="group block overflow-hidden rounded-3xl border border-dashed border-blue-300 bg-gradient-to-br from-blue-50 via-white to-blue-50/50 hover:shadow-lg transition-all"
    >
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-5 sm:p-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-blue-700 text-white grid place-items-center shadow-md shrink-0">
          ...
          <div className="min-w-0">
            ...
            <p className="font-display font-black text-slate-900 text-lg sm:text-xl leading-tight truncate">
              Sua marca em destaque na IndiqueLeads
            </p>
          </div>
        </div>
        <div className="sm:ml-auto flex flex-col sm:items-end gap-1">
          ...
          <span className="inline-flex items-center gap-1 text-xs font-bold text-blue-800 group-hover:underline">
...
function SponsorCardItem({ sponsor }: { sponsor: Sponsor }) {
  ...
  const className =
    "block rounded-2xl border border-slate-200 bg-white p-3 hover:border-blue-300 hover:shadow-md transition-all";
        ...
        <div className="flex flex-wrap gap-2">
          {sponsors!.map((s) => (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-700 hover:border-blue-300 hover:text-blue-800"
            >
